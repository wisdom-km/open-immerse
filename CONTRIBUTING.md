...
- `pdf/`：扩展自有 pdf.js 阅读器。打开本地/链接 PDF、左侧连续滚动（上一页/下一页仍可用）、缩放；翻译范围默认当前页，可选全文（按页顺序走 `OI_TRANSLATE_BATCH`，可停止，不整本一次送模型）。右侧累计已译页的连续中文（`.pane-translate .readout > h1.oi-pdf-h1 / h2.oi-pdf-h2 / p.oi-pdf-p`，节点带 `data-page`），空态 `.empty-read`「点击翻译」，不按块收藏、默认不展示原文摘录。左侧滚动时若该页已译，右侧弱同步到对应段落。已译页缓存在阅读器内；原文清空当前页译文。不做 OCR、也不注入 Chrome PDF Viewer。
