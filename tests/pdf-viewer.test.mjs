...
test("M1 viewer is an extension-owned pdf.js page, not Chrome PDF injection", () => {
  ...
  assert.equal(html.includes("split-handle"), false);
...
test("split layout is left/right by default and stacks below 900px", () => {
  ...
  assert.equal(css.includes(".split-handle"), false);
  assert.equal(css.includes("col-resize"), false);
  assert.equal(css.includes(".zoom-stack"), false);
  assert.equal(src.includes("bindSplitResize"), false);
  assert.equal(src.includes("startSplitDrag"), false);
  assert.equal(src.includes("applySplit"), false);
