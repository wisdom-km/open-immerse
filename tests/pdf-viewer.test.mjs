...
test("split layout is left/right by default and stacks below 900px", () => {
  ...
  assert.match(css, /\.split-handle::before\s*\{[^}]*width:\s*1px[^}]*background:\s*color-mix\(in srgb, var\(--oi-line\)/s);
  assert.match(css, /\.split-handle:hover::before\s*\{[^}]*width:\s*2px[^}]*var\(--oi-accent\)/s);
  assert.match(css, /\.split-handle:active::before\s*\{[^}]*var\(--oi-accent\)[^}]*box-shadow:/s);
  assert.match(css, /\.split-handle::after\s*\{[^}]*opacity:\s*0/);
  assert.match(css, /\.split-handle:hover::after\s*\{[^}]*opacity:\s*1/);
