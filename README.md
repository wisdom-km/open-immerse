...
当前版本 **0.3.0**。V1 先做好网页双语。学习中心与文档翻译可用。**PDF 阅读是实验室功能：默认关闭**，只在 **设置 → 高级** 里手动打开后才会出现入口。目标是：文字层正文 + 原页内容精准展示（默认裁图）；公式画面不来自 LaTeX 渲染。不是把译文贴进左栏。YouTube / X 字幕、悬浮翻译、划词卡片代码还在，**默认关闭，不出现在主界面**。
...
| PDF 阅读（实验室） | **默认关闭**（设置 → 高级，opt-in）。目标：文字层正文 + 原页内容精准展示（默认裁图）；公式画面不来自 LaTeX 渲染。公式、图、表跟打开的 PDF 是同一套内容。有文字层的正文用文字层原句。译文只在右栏。测试期划区用本机智谱 GLM-OCR，上线用 OCR API 加已配置的大模型 API。**网页双语仍是主产品。** |
...
主卡片：「翻译此页」（进行中变为「停止」）、「恢复原文」、本站自动、范围与语言。额度在折叠的「更多」。**没有进度条。** 底栏是 **设置 · 学习 · 文档**。默认底部没有 PDF；开启实验室 PDF 后，底栏才会出现 PDF。当前标签已是 PDF 时才会出现「在沉浸译中打开（实验室）」。

**设置四组：引擎 / 阅读 / 功能 / 高级**

左侧栏标题下是弱化的 **学习中心 · 文档翻译**（不是强调色按钮），其下四个分组。

![设置：默认引擎与测试连接](docs/screenshots/03-options-engine.png)

**引擎**：当前默认引擎的密钥、Base URL、模型与测试连接（截图已打码）。

![设置：阅读](docs/screenshots/04-options-reading.png)

**阅读**：只有译文字号比例、正文译文间距、译文段间距。没有译文样式下拉或预览表。

![设置：左侧导航](docs/screenshots/07-options-nav.png)

左侧栏：弱化的「学习中心 · 文档翻译」+ 引擎 / 阅读 / 功能 / 高级。

![设置：高级](docs/screenshots/08-options-advanced.png)

**高级**：悬浮 / 划词 / 字幕与 **PDF 阅读（实验室）** 默认关。**先信后润** 在 **功能** 组（默认开），不在这里。
...
弹层里还可以改：本页范围、源/目标语言、本站自动；本次额度在「更多」。底栏是 **设置 · 学习 · 文档**。默认不出现 PDF 入口。只有在 **设置 → 高级** 打开「PDF 阅读（实验室）」后，弹层底部才会出现 **PDF**；当前标签已是 PDF 时才会出现「在沉浸译中打开（实验室）」。
...
**不是默认入口。** 先到 **设置 → 高级** 勾选「PDF 阅读（实验室）」才会出现弹层底部 **PDF**，以及 PDF 标签上的「在沉浸译中打开（实验室）」。
...
- 不注入 Chrome 自带 PDF Viewer。没有可信文字层时，正文才用识别结果，并标明可能有误差；公式、图、表仍是原页裁图
- 网页双语仍是主产品与回归标准
...
| 项 | 说明 |
| --- | --- |
| **PDF 阅读（实验室，默认关）** | `features.pdf` 默认 `false`。目标：文字层正文 + 原页内容精准展示（默认裁图）；公式画面不来自 LaTeX 渲染。有文字层的正文用文字层原句。译文只在右栏。测试期划区用本机智谱 GLM-OCR，上线用 OCR API 加已配置的大模型 API。网页双语仍是主产品 |
| 侧栏英/中配对 | 搁置；产品焦点是正文双语 |
| Chrome 网上应用店 | 目前只支持加载已解压目录 |
| OCR | 无可信文字层时，正文才用识别结果，并标明可能有误差。公式、图、表仍是原页裁图。把 `i` 认成 `n` 或把 `n` 认成 `i` 不上屏 |
