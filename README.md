...
| PDF 阅读 | 扩展内 pdf.js 打开本地/链接 PDF；左侧连续滚动；翻译当前页或按页顺序译全文；右侧按原页 bbox **版式镜像**（作者格/标题/摘要定位；图与公式为原页裁剪）。无文字层不编造镜像。导出 MD/PDF 仍为纯文本 |
...
5. 当前页是 PDF 时，点弹层「在沉浸译中打开」；也可从弹层底部 PDF 进入后选择本地文件。左侧用滚轮连续翻页。顶栏 **当前页 | 全文** 默认当前页，点 **翻译** 开译（全文按页顺序，进度「翻译中 · k/n」，仅翻译中显示 **停止**；停止或译完后主按钮立刻回 **翻译**，已译中文保留）。翻译中右侧不再停在「点击翻译」。**原文只清空当前页译文，不清全文**；右侧累计已译页的连续中文，标题作标题、正文作段落
