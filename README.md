...
主按钮「翻译此页」（进行中变为「停止」）、「恢复原文」、范围与语言。额度在折叠的「更多」。**默认底部没有 PDF**；上图若仍显示旧开关/底部 PDF，以本文信息架构为准——底部 PDF 链只在**开启实验室 PDF 后**出现。
...
设置页四组：**引擎**（密钥与测试连接）、**阅读**（字号与正文间距）、**功能**（网页 / FAB / 学习 / 文档 / 先信后润）、**高级**（PDF 实验室等）。截图若仍是单页长表，以四组导航为准。
...
弹层里还可以改：本页范围、源/目标语言、本站自动；本次额度在「更多」。底部是 **设置 · 学习 · 文档**。默认不出现 PDF 入口。只有在 **设置 → 高级** 打开「PDF 阅读（实验室）」后，弹层底部才会出现 **PDF**；当前标签已是 PDF 时才会出现「在沉浸译中打开（实验室）」。

译文字号比例和正文间距在 **设置 → 阅读**。不再提供译文样式下拉或预览表。
