...
| 模块 | 现状 |
| --- | --- |
| 页面开关按钮 | 右下角玻璃胶囊：翻译 / 停止、原文、‹ 收起。仅翻译中（含润色）显示停止；停止只中止 inflight、保留译文。**原文**才清译文 |
| 网页翻译 | 默认仅正文（主栏，不含侧栏/导航）。全页面含侧栏；页头主 H1 必译，顶栏导航链仍可跳过 |
| 学习中心 | 收藏单词/短语/句子；全部 / 今日待复习；导出 Markdown / PDF / Word |
| 文档翻译 | TXT/MD/HTML 直读双语、译文可改、导出 HTML |
| PDF 阅读 | 扩展内 pdf.js 打开本地/链接 PDF；左侧连续滚动；翻译当前页或按页顺序译全文；右侧默认按原页 bbox **版式镜像**（作者格/标题/摘要定位；公式为可复制 LaTeX + KaTeX，图为原页裁剪），可切次级「通读」。无文字层不编造镜像。导出 MD 含 `$...$` / `$$...$$` |
...
5. 当前页是 PDF 时，点弹层「在沉浸译中打开」；也可从弹层底部 PDF 进入后选择本地文件。左侧用滚轮连续翻页。顶栏 **当前页 | 全文** 默认当前页，点 **翻译** 开译（全文按页顺序，进度「翻译中 · k/n」，仅翻译中显示 **停止**；停止或译完后主按钮立刻回 **翻译**，已译中文保留）。翻译中右侧不再停在「点击翻译」。**原文只清空当前页译文，不清全文**；右侧默认按页镜像已译块（标题/作者/摘要各就各位），「通读」才是连续中文
