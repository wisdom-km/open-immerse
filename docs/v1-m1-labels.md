...
- 数学 Unicode：公式，0.90。
- 等宽字体：代码，0.90。
- 更小、并且基线相对紧挨着的前一个基字母上移或下移的上下标，并进那个基字母，合成一个单元。新接上的上下标置信度 0.74（规则 `script-attached`）。旁边已经是数学字体时，仍用原来的 0.82（`script-offset`），不抬到 0.96。
- 同一行上紧挨着公式的运算符、括号、数字和分数线，并进该公式（`math-run` 0.70，分数线仍是 0.80）。
- 正文词或标点正后方的小号上移数字（含逗号、短横、*、†、‡），行首脚注号，以及和本页参考文献编号相同的这类上标：文字，不是公式。置信度 0.84（`citation-or-footnote`）。`x_i`、`a^4` 这种紧贴短符号的指数仍是公式。
- 没有基字母、只是单独漂着的弱上下标：公式，0.62，会进复核队列。
- 其余正文：文字，0.93。
- 很大的路径和图片：其他。
- 行末、靠右、形如 `(12)` 的正文，并和公式同一行：公式编号，并进那个行间单元。
- 单元置信度是成员里的最低值。低于 0.65 记为 fallback，复核时优先看。置信度不会为了少进队列而被抬高。
