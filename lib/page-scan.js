...
/** Stamp live label hash on the collected node AND the mount host (title-container). */
export function rebindLiveSourceHash(source) {
  const liveHash = sourceTextHash(getText(source));
  setAttr(source, "data-oi-src-hash", liveHash);
  const host = pickSideRailMountHost(source);
  if (host && host !== source) setAttr(host, "data-oi-src-hash", liveHash);
  return liveHash;
}
...
export function detachStaleRailGloss(source) {
  ...
  const stale = nodes.some((node) => attr(node, "data-oi-src-hash") !== liveHash);
  const removed = [];
  if (stale) {
    nodes.forEach((node) => {
