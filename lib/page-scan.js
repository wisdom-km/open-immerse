...
export function collectMountedGloss(source) {
  ...
  if (typeof source.querySelectorAll === "function") {
    const list = source.querySelectorAll(".oi-translation");
...
/**
 * Virtual-list reuse: same row DOM, new label. Drop leftover .oi-translation
 * whose source-text key no longer matches. Do not keep the previous item's gloss.
 */
export function detachStaleRailGloss(source) {
  ...
  collectMountedGloss(source).forEach((node) => {
    ...
    if (nodeHash === liveHash) return;
    detachGlossNode(node);
    removed.push(node);
  });
  setAttr(source, "data-oi-src-hash", liveHash);
  return removed;
}
...
export function pairGlossBySource(sources, translations, keep) {
  ...
  list.forEach((el, i) => {
    ...
    setAttr(el, "data-oi-src-hash", srcHash);
  ...
  return targets.map((el) => {
    ...
    const srcHash = attr(el, "data-oi-src-hash") || sourceTextHash(getText(el));
    let text = "";
