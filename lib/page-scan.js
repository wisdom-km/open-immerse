...
export function planTranslationMount(el, ctx = {}, opts = {}) {
  ...
  const placement = inline || tableCell || (!forceBlock && listLike) ? "append" : "afterend";
  return {
...
export function placeTranslationNode(source, node, plan) {
  ...
  if (plan.placement === "append" && source.appendChild) {
    source.appendChild(node);
    return node;
  }
  if (source.insertAdjacentElement) {
    source.insertAdjacentElement("afterend", node);
    return node;
  }
  source.parentElement?.insertBefore?.(node, source.nextElementSibling || null);
  return node;
}
