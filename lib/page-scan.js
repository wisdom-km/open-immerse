...
/** aside / complementary / geometric left-right rail. Page scope keeps these; article still skips. */
export function inSideRail(el, ctx = {}) {
  if (!el) return false;
  if (el.closest?.("aside, [role='complementary']")) return true;
  return isSideColumn(el, ctx.innerWidth);
}
...
/** Class used by content.js mountTranslation. Side rails stay block, never oi-inline. */
export function translationClassName(el, ctx = {}) {
  if (shouldInline(el, ctx)) return "oi-translation oi-inline";
  if (/^H[1-3]$/.test(String(el?.tagName || "").toUpperCase())) return "oi-translation oi-after-heading";
  return "oi-translation";
}
