...
import { fontNameForFormula, isMathFontName } from "./pdf-mirror.js";
...
const BODY_FACE = /Nimbus|Times|Helvetica|Arial|Courier|Calibri|Georgia|Liberation|TeXGyre|CMR\d/i;

function centerOf(box) {
  return [(box[0] + box[2]) / 2, (box[1] + box[3]) / 2];
}

function isBodyFace(font) {
  return BODY_FACE.test(String(font || "")) && !/Math/i.test(String(font || ""));
}

function glyphChar(run, box) {
  if (!run) return "";
  const str = String(run.str || "");
  if (str.length <= 1) return str;
  const width = run.box[2] - run.box[0];
  if (!(width > 0)) return str.slice(0, 1);
  const t = (((box[0] + box[2]) / 2) - run.box[0]) / width;
  const index = Math.min(str.length - 1, Math.max(0, Math.floor(t * str.length)));
  return str[index] || "";
}

function runBaseline(item, viewport) {
  const transform = Array.isArray(item?.transform) ? item.transform : [];
...
/** Text runs in page pixels, tagged as formula members or body lines. */
export function buildFormulaRuns(items, viewport, formula) {
  const pageWidth = Number(viewport?.width) || Number(formula?.pageWidth) || 0;
  const pageHeight = Number(viewport?.height) || Number(formula?.pageHeight) || 0;
  const own = (formula?.glyphBoxes || []).map((box) => pageBox(box, pageWidth, pageHeight)).filter(Boolean);
  const runs = [];
  for (const item of items || []) {
    const str = String(item?.str || "");
    if (!str.trim()) continue;
    const box = pageBox(textItemNormBox(item, viewport), pageWidth, pageHeight);
    const area = boxArea(box);
    if (!(area > 0)) continue;
    const font = fontNameForFormula(item);
    const math = isMathFontName(font);
    const member = own.some((glyph) => overlapArea(box, glyph) / area > 0.5);
    runs.push({
      box,
      font,
      math,
      body: isBodyFace(font) && !math,
      baseline: runBaseline(item, viewport),
      str,
      member
    });
  }
...
function bestRun(box, runs) {
  const area = Math.max(boxArea(box), 1e-6);
  const [cx, cy] = centerOf(box);
  let best = null;
  let bestScore = 0;
  for (const run of runs || []) {
    const overlap = overlapArea(box, run.box);
    const [rx, ry] = centerOf(run.box);
    const coversRun = rx >= box[0] && rx <= box[2] && ry >= box[1] && ry <= box[3];
    const centerInRun = cx >= run.box[0] - 0.6 && cx <= run.box[2] + 0.6 &&
      cy >= run.box[1] - 0.6 && cy <= run.box[3] + 0.6;
    if (!(overlap > 0 || coversRun || centerInRun)) continue;
    const glyphShare = overlap / area;
    const runShare = overlap / Math.max(boxArea(run.box), 1e-6);
    let score = glyphShare + runShare * 0.35;
    if (centerInRun) score += 0.55;
    if (coversRun) score += 0.2;
    if ((run.member || run.math) && (centerInRun || glyphShare >= 0.45)) score += 0.12;
    if (score > bestScore) {
      bestScore = score;
      best = run;
    }
...
/**
 * Positive membership. Keep a glyph when its text run is a formula member,
 * when its math face meets the formula, or when its ink covers the formula
 * cluster. Tall operators stay even if they extend past the line band.
 * Drop a glyph only when its run is body text and not a formula member.
 */
export function decideGlyph(element, context) {
  const box = paintBox(element);
  const area = Math.max(boxArea(box), 1e-6);
  const glyphs = context.glyphs || [];
  const cluster = unionBox(glyphs);
  const unit = context.unit;
  const run = bestRun(box, context.runs);
  const clusterPad = cluster ? expandBox(cluster, 2) : null;
  const overlapCluster = clusterPad ? overlapArea(box, clusterPad) : 0;
  const inCluster = overlapCluster / area;
  const coversMember = glyphs.some((glyph) => {
    const [gx, gy] = centerOf(glyph);
    return gx >= box[0] && gx <= box[2] && gy >= box[1] && gy <= box[3];
  });
  const overlapsMember = bestOverlap(box, glyphs) > 0;
  const char = glyphChar(run, box);
  const font = run?.font || "";
  const bodyLine = Boolean(run && run.body && !run.math && !run.member);
  const meta = { run, char, font, bodyLine, inCluster };
  if (bodyLine) return { keep: false, reason: "body-line", ...meta };
  const touchesFormula = overlapsMember || coversMember || overlapCluster > 0.5;
  const runShare = run ? overlapArea(box, run.box) / area : 0;
  const unitShare = unit ? overlapArea(box, expandBox(unit, 2)) / area : 0;
  if (run?.member && (touchesFormula || runShare >= 0.3 || unitShare >= 0.3)) {
    return { keep: true, reason: "formula-run", ...meta };
  }
  if (run?.math && (touchesFormula || unitShare >= 0.15)) {
    return { keep: true, reason: "math-font", ...meta };
  }
  if (!run?.body && (coversMember || (overlapsMember && inCluster >= 0.35))) {
    return { keep: true, reason: "glyph-cluster", ...meta };
  }
  if (!run?.body && bestOverlap(box, glyphs) > 0.4) {
    return { keep: true, reason: "glyph-cluster", ...meta };
  }
  if (!run && unit && overlapArea(box, unit) / area >= 0.3) {
    return { keep: true, reason: "glyph-cluster", ...meta };
  }
  if (!run && unit && overlapArea(box, unit) === 0) {
    const beside = overlapArea(box, expandBox(unit, 2));
    if (beside > 0 && beside < area * 0.5) {
      return { keep: false, reason: "body-line", ...meta, bodyLine: true };
    }
  }
  return { keep: false, reason: run ? "outside-formula-glyphs" : "unclaimed", ...meta };
}

const BODY_LEAK_FACE = /Nimbus|Times|Helvetica|Arial|Courier|Calibri|Georgia|Liberation|TeXGyre/i;

/** A kept glyph counts as body text when it is tagged body-line, or its face is a text face and the run is not a formula member. */
export function isBodyGlyphLeak(element) {
  if (!element || element.provenance !== "glyph") return false;
  if (element.bodyLine) return true;
  const font = String(element.font || "");
  const bodyFace = BODY_LEAK_FACE.test(font) && !/Math/i.test(font);
  return bodyFace && element.reason !== "formula-run";
}

function attachRun(element, decision) {
  return {
    ...element,
    reason: decision.reason,
    font: decision.font || "",
    text: decision.char || "",
    bodyLine: decision.bodyLine === true
  };
}

/**
 * Keep formula text runs, math faces, and ink that covers the formula
 * cluster. Drop only body text on the line above or below.
 * Rules stay at the length pdf.js painted.
 */
export function selectFormulaElements(record, formula) {
  ...
  const context = { glyphs, unit, runs: formula?.runs || [] };
  const selected = [];
  const rejected = [];
  for (const element of record?.elements || []) {
    ...
    if (element.provenance === "glyph") {
      const decision = decideGlyph(element, context);
      const tagged = attachRun(element, decision);
      if (decision.keep) selected.push(tagged);
      else if (overlapArea(box, grown) > 0 || decision.reason === "body-line") {
        rejected.push({
          id: element.id,
  ...
  if (!selected.some((element) => element.provenance === "glyph")) {
    let best = null;
    let bestAmount = 0;
    for (const element of record?.elements || []) {
      if (element?.provenance !== "glyph") continue;
      const amount = bestOverlap(paintBox(element), glyphs);
    ...
    if (best && bestAmount > 0) {
      selected.push({ ...best, reason: "glyph-fallback", font: "", text: "", bodyLine: false });
    }
  }
  let view = unit.slice();
  const ink = unionBox(selected.map((element) => paintBox(element)));
  if (ink) {
    const limit = expandBox(unit, 24);
    view = [
...
/** Glyph ids the 20487d4 selector kept. Used only to justify later removals. */
export function glyphIdsAt20487d4(record, formula) {
  const pageWidth = Number(formula?.pageWidth) || Number(record?.width) || 0;
  const pageHeight = Number(formula?.pageHeight) || Number(record?.height) || 0;
  const glyphs = (formula?.glyphBoxes || []).map((box) => pageBox(box, pageWidth, pageHeight)).filter(Boolean);
  const foreign = (formula?.foreignBoxes || []).map((box) => pageBox(box, pageWidth, pageHeight)).filter(Boolean);
  ...
  for (const element of record?.elements || []) {
    ...
    const onFormula = bestOverlap(box, glyphs);
    const onForeign = bestOverlap(box, foreign);
    const inUnit = overlapArea(box, grown);
    const keep = (onFormula > 0.4 && onFormula >= onForeign) ||
      (inUnit >= Math.min(area * 0.45, 8) && inUnit >= onForeign);
    if (keep) ids.push(element.id);
  }
  return ids;
}
...
export function buildFormulaSvg(record, formula) {
  ...
  for (const element of selection.elements) {
    ...
    listed.push({
      ...
      font: element.font || "",
      text: element.text || "",
      bodyLine: element.bodyLine === true,
