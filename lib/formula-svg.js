...
/** Missing-ink tolerance against a FontFace reference. */
...
/**
 * Keep glyph outlines that sit on this unit's glyph boxes, plus vector
 * rules (fraction bars, radical lines) that live inside the unit and not
 * on a neighboring glyph.
 */
export function selectFormulaElements(record, formula) {
  ...
  for (const element of record?.elements || []) {
    ...
    if (element.provenance === "glyph") {
      ...
      if (keep) selected.push({ ...element, reason: onFormula > 0.4 ? "glyph-membership" : "glyph-inside-unit" });
    ...
    const clipped = clipStrokeToRect(element, grown);
    if (clipped) {
      const clippedForeign = bestOverlap(paintBox(clipped), foreign);
      if (clippedForeign < Math.max(boxArea(paintBox(clipped)) * 0.45, 0.8)) {
        selected.push({ ...clipped, reason: "path-clipped-to-unit" });
        continue;
      }
    }
    const mostlyInside = inside >= 0.55 && area <= boxArea(grown) * 1.25;
    const onInk = onFormula > 0.4 && onFormula >= onForeign;
    const keep = (mostlyInside || onInk) && onForeign < Math.max(onFormula, area * 0.25);
...
export function countMissingInk(reference, svg, width, height, mask, tolerance = INK_AA_TOLERANCE) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (lumaOf(reference, pixel * 4) >= refInk) continue;
      referenceInk += 1;
      if (!inRadius(svg, width, height, x, y, radius, cover)) missing += 1;
    }
  }
  return { missing, referenceInk };
}
