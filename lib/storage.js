...
import { normalizePdfScroll } from "./pdf-scroll.js";

export const SETTINGS_VERSION = 8;

/** Optical gap (em) between a body paragraph and its block translation. Headings are not affected. */
export const BODY_GLOSS_GAP_MIN = 0.1;
export const BODY_GLOSS_GAP_MAX = 0.7;
export const BODY_GLOSS_GAP_DEFAULT = 0.35;

export function normalizeBodyGlossGap(value) {
  const n = parseFloat(value);
  if (!Number.isFinite(n)) return BODY_GLOSS_GAP_DEFAULT;
  return Math.round(Math.min(BODY_GLOSS_GAP_MAX, Math.max(BODY_GLOSS_GAP_MIN, n)) * 100) / 100;
}

/** Extra vertical gap (em) between consecutive body glosses. Headings are not affected. */
export const BODY_GLOSS_STACK_GAP_MIN = 0;
export const BODY_GLOSS_STACK_GAP_MAX = 2.5;
export const BODY_GLOSS_STACK_GAP_DEFAULT = 0.25;

export function normalizeBodyGlossStackGap(value) {
  const n = parseFloat(value);
  if (!Number.isFinite(n)) return BODY_GLOSS_STACK_GAP_DEFAULT;
  return Math.round(Math.min(BODY_GLOSS_STACK_GAP_MAX, Math.max(BODY_GLOSS_STACK_GAP_MIN, n)) * 100) / 100;
}
...
export const DEFAULT_SETTINGS = {
  ...
  twoStepPolish: true, // 「先信后润」; default on; missing key → true unless user saved false
  deepThink: false, // Advanced「深度思考」; default off (Ark/DeepSeek stay fast)
  fontScale: 0.95,
  bodyGlossGap: BODY_GLOSS_GAP_DEFAULT,
  bodyGlossStackGap: BODY_GLOSS_STACK_GAP_DEFAULT,
  pdfLayout: {
    mode: "",
...
export function migrateSettings(merged, stored = {}) {
  ...
  if (version < 8) {
    next.settingsVersion = SETTINGS_VERSION;
    if (!Object.prototype.hasOwnProperty.call(stored, "twoStepPolish")) {
      next.twoStepPolish = true;
    } else {
      next.twoStepPolish = stored.twoStepPolish === true;
    }
    changed = true;
  }
  if (
...
function merge(base, patch) {
  ...
  const pdfScrollPatch = patch.pdfScroll && typeof patch.pdfScroll === "object" ? patch.pdfScroll : {};
  out.pdfScroll = normalizePdfScroll({ ...(base.pdfScroll || {}), ...pdfScrollPatch });
  return out;
}
