...
export function installRecordingPath2D() {
  const current = globalThis.Path2D;
  if (current && current.__oiRecording) return current;
  if (typeof current === "function") globalThis.__oiNativePath2D = current;
...
export class SvgRecorder {
  ...
  lineTo(x, y) {
    this.path.push({ t: "L", x, y });
  }

  bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y) {
    this.path.push({ t: "C", x1: cp1x, y1: cp1y, x2: cp2x, y2: cp2y, x, y });
  }

  quadraticCurveTo(cpx, cpy, x, y) {
    this.path.push({ t: "Q", x1: cpx, y1: cpy, x, y });
  }
  ...
  _opsFrom(path) {
    if (path && Array.isArray(path._ops)) {
      ...
      return { ops: path._ops.map(cloneOp), source: path._source || "path" };
    }
    if (path) {
      this.noteUnsupported("Path2D.unreadable");
      return { ops: [], source: "path" };
    }
    return { ops: this.path.map(cloneOp), source: "path" };
  }

  _remember(op, ops, source, style) {
    if (!ops.length || ops.every((item) => item.t === "Z")) return;
    const canvasOps = transformOps(ops, this.ctm);
  ...
  fill(pathOrRule, rule) {
    ...
    const { ops, source } = this._opsFrom(path);
    ...
    this._remember("fill", ops, source, style);
  }

  stroke(path) {
    const explicit = path && typeof path !== "string" ? path : null;
    const { ops, source } = this._opsFrom(explicit);
    const style = this._style("stroke");
    if (this.globalAlpha === 0 || (style.color && style.color.a === 0)) return;
    if (!style.color) style.css = "#000000";
    this._remember("stroke", ops, source, style);
  }
