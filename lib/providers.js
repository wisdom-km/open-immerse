 ...
 *
 * All LLM adapters share one translation Skill (lib/translate-skill.js).
 * Model/vendor come from user settings — never from the Skill.
 */
...
/** Shared Skill prompt used by every LLM adapter. Custom settings.prompt still wins. */
...
/**
 * zh-CN safety net for the known "business owners" → 主教 fusion.
 * Rewrites 小型企业主教 / 小企业主教 / 企业主教 unless the paired source
 * looks religious. Does not require "business owners" on the same index
 * (parseNumbered can misalign, e.g. source "Lesson 1").
 *
 * Do not strip 教 when it starts a teach-verb (教会 / 教导 / 教给 / 教我们 / 教了).
 * `小企业主教会我们` is 小企业主+教会我们, not 主教.
 */
export function guardZhBusinessSense(texts, translations, targetLang) {
  ...
  // Always rewrite the known bad fusion for zh-CN unless the SOURCE is actually religious.
  // Do not require "business owners" on the same index — parseNumbered misalignment can pair wrong.
  return translations.map((dst, i) => {
    const src = String(texts?.[i] || "");
    const out = String(dst || "");
    if (!/主教/.test(out)) return out;
    if (/\b(bishop|bishops|church|catholic|priest|pope|diocese|clergy)\b/i.test(src)) return out;
    return out
      .replace(/小型企业主教(?!会|导|给|我们|了)/g, "小企业主")
      .replace(/小企业主教(?!会|导|给|我们|了)/g, "小企业主")
      .replace(/企业主教(?!会|导|给|我们|了)/g, "企业主");
  });
}

/**
 * Narrow zh title/heading safety net for known hollow calques.
 * Only rewrites short, heading-like lines (no sentence punctuation).
 * Does not invent a stock gloss — strips the banned wrapper and keeps the topic.
 */
export function guardZhTitleCalques(texts, translations, targetLang) {
  if (!isChineseTarget(targetLang) || !Array.isArray(translations)) return translations;
  return translations.map((dst) => {
    const out = String(dst || "");
    if (!/让我们了解到|关于.+哪些内容|哪些关于.+的内容/.test(out)) return out;
    if (!looksLikeZhHeading(out)) return out;
    return tightenZhTitleCalques(out);
  });
}

/** Compose zh safety nets. Used after every LLM parse and again in the SW batch path. */
export function guardZhTranslations(texts, translations, targetLang) {
  return guardZhTitleCalques(
    texts,
    guardZhBusinessSense(texts, translations, targetLang),
    targetLang
  );
}

function looksLikeZhHeading(text) {
...
export function tightenZhTitleCalques(text) {
  let out = String(text || "");
  // 「让我们了解到了关于X的哪些内容」and the swapped 「让我们了解到了哪些关于X的内容」
  out = out.replace(/让我们了解到了?的?关于(.+?)的哪些内容/g, "给我们的$1");
  out = out.replace(/让我们了解到了?的?哪些关于(.+?)的内容/g, "给我们的$1");
  out = out.replace(/让我们了解到了?的?/g, "给我们的");
  out = out.replace(/关于(.+?)的哪些内容/g, "的$1");
  out = out.replace(/哪些关于(.+?)的内容/g, "的$1");
  out = out.replace(/的{2,}/g, "的");
  out = out.replace(/^的+/g, "");
  return out;
}

function parseLlmTranslations(raw, texts, targetLang) {
  return guardZhTranslations(texts, parseNumbered(raw, texts.length), targetLang);
}

/**
 * Shared LLM path: single-shot by default; two-step (信 draft → 达雅 polish)
 * when top-level twoStepPolish=true and there is no custom prompt.
 * `complete({ system, user })` returns the raw model text.
 */
export function appendPromptAddendum(prompt, addendum) {
  const extra = String(addendum || "").trim();
  if (!extra) return String(prompt || "");
  return `${prompt}\n\n${extra}`;
}

async function translateWithSkill(texts, ctx, complete) {
  const skillPrompt = appendPromptAddendum(resolveTranslatorPrompt(ctx), ctx.settings?.promptAddendum);
  const steps = isTwoStepPolish(ctx.settings) ? ["faithful", "polish"] : ["single"];
  let drafts = [];
  for (const step of steps) {
    const turn = buildLlmTurn({ texts, ctx, skillPrompt, step, drafts });
    try {
      const raw = await complete(turn);
      drafts = parseLlmTranslations(raw, texts, ctx.targetLang);
    } catch (err) {
      if (step === "polish" && drafts.some((t) => String(t || "").trim())) {
        const wrapped = err instanceof Error ? err : new Error(String(err || "润色失败"));
        wrapped.polishFailed = true;
        wrapped.drafts = drafts;
        throw wrapped;
      }
      throw err;
    }
    if (step === "faithful") {
      try {
        await ctx.onProgress?.({ phase: "draft", translations: drafts });
      } catch {
        /* progress is best-effort; polish still runs */
      }
    }
  }
  return drafts;
...
/** Top-level deepThink (default false). Translation stays fast unless the user opts in. */
export function isThinkingEnabled(settings) {
  return settings?.deepThink === true;
}

const OTHER_ENGINE_BASE_RE =
  /api\.openai\.com|moonshot\.|minimax\.|api\.x\.ai|openrouter\.ai|anthropic\.com|googleapis\.com|openai\.azure\.com/i;
const ARK_DEEPSEEK_BASE_RE = /volces\.com|volcengine\.com|api\.deepseek\.com/i;
const DEEPSEEK_MODEL_RE = /deepseek/i;

/**
 * Only Ark / DeepSeek V4 Flash get thinking fields.
 * Other engines must omit them (no-op, never 400 on unknown keys).
 */
export function shouldAttachThinking(settings = {}, body = {}) {
  const base = String(settings.baseUrl || "").toLowerCase();
  const model = String(body.model || settings.model || "").toLowerCase();
  if (OTHER_ENGINE_BASE_RE.test(base)) return false;
  return ARK_DEEPSEEK_BASE_RE.test(base) || DEEPSEEK_MODEL_RE.test(model);
}

/**
 * Ark / DeepSeek thinking controls for Chat Completions.
 * Default off: thinking.type=disabled + reasoning_effort=minimal
 * (flash variants that ignore `thinking` still get a low-effort fallback).
 * Non-Ark engines: leave the body untouched.
 */
export function applyChatThinking(body, settings) {
  const next = body && typeof body === "object" ? body : {};
  if (!shouldAttachThinking(settings, next)) {
    delete next.thinking;
    delete next.reasoning_effort;
    return next;
  }
  const enabled = isThinkingEnabled(settings);
  next.thinking = { type: enabled ? "enabled" : "disabled" };
  if (enabled) {
    delete next.reasoning_effort;
  } else {
...
async function chatComplete(ctx, opts, { system, user }, temperature = 0.2) {
...
async function translateChatCompletions(texts, ctx, opts) {
  return translateWithSkill(texts, ctx, (turn) => chatComplete(ctx, opts, turn));
}
...
function openaiCompatProvider({ id, name, defaultBase, defaultModel, extraFields = [], extraHeaders }) {
  return {
    ...
    async complete(turn, ctx) {
      const headers = typeof extraHeaders === "function" ? extraHeaders(ctx) : extraHeaders;
      return chatComplete(ctx, {
        label: name,
        fallbackBase: resolveBase(ctx, defaultBase),
        fallbackModel: defaultModel,
        extraHeaders: headers
...
export const providers = {
  ...
  gemini: {
    ...
    async translate(texts, ctx) {
      ...
      return translateWithSkill(texts, ctx, async ({ system, user }) => {
        const res = await postJson(url, {
    ...
    async complete({ system, user }, ctx) {
      const model = ctx.settings.model || "gemini-2.0-flash";
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(ctx.settings.apiKey)}`;
      const res = await postJson(url, {
...
/** One strict JSON completion on the active provider. Machine translators cannot do this. */
export async function completePdfStructure(settings, { system, user } = {}) {
  const provider = getProvider(settings?.provider);
  if (typeof provider.complete !== "function") {
    return { ok: false, error: "structure unsupported" };
  }
  try {
    const raw = await provider.complete(
      { system: String(system || ""), user: String(user || "") },
      {
...
export function getProvider(id) {
  const provider = providers[id] || providers.mymemory;
  return {
    ...provider,
    async translate(texts, ctx = {}) {
      try {
        const out = await provider.translate(texts, ctx);
        return guardZhTranslations(texts, Array.isArray(out) ? out : [], ctx.targetLang);
      } catch (err) {
        if (err?.polishFailed && Array.isArray(err.drafts)) {
          err.drafts = guardZhTranslations(texts, err.drafts, ctx.targetLang);
        }
        throw err;
      }
    }
...
export function isProviderConfigured(provider, cfg = {}) {
  return (provider.fields || [])
    .filter((field) => field.required)
    .every((field) => String(cfg[field.key] ?? "").trim() !== "");
}

/** Short English probe used by the options-page connection test. */
export const PROVIDER_PROBE_TEXT = "Hello";

const SECRET_FIELD_RE = /key|token|secret|password|authorization|headersjson/i;

export function truncateStatus(text, max) {
  const s = String(text || "");
  if (s.length <= max) return s;
  return `${s.slice(0, max)}…`;
}

/** Redact keys before any options-page status text hits the DOM. */
...
/**
 * Strip secrets from provider errors. Never echo an API key in UI or logs.
 */
export function sanitizeProviderError(err, settings = {}) {
  let msg = String(err && err.message != null ? err.message : err || "未知错误");
  const secrets = new Set();
  for (const [key, value] of Object.entries(settings || {})) {
    const text = String(value || "").trim();
    if (text.length < 6) continue;
    if (SECRET_FIELD_RE.test(key)) secrets.add(text);
  }
...
export function missingRequiredField(provider, cfg = {}) {
  return (provider.fields || []).find((field) => field.required && String(cfg[field.key] ?? "").trim() === "");
}

export function formatConnectionStatus(result, probeText = PROVIDER_PROBE_TEXT) {
  if (result?.ok) {
    return `连接成功 · ${probeText} → ${truncateStatus(redactProviderText(result.translation || ""), 24)}`;
  }
  if (result?.missingField) {
    return `请先填写 ${result.missingField}`;
  }
  const short = truncateStatus(result?.error || "未知错误", 80);
  if (result?.httpStatus) {
    const rest = short.replace(/\s*HTTP\s+\d{3}\s*/g, " ").replace(/^[·\s]+|[·\s]+$/g, "").trim();
    return rest ? `连接失败 · HTTP ${result.httpStatus} · ${rest}` : `连接失败 · HTTP ${result.httpStatus}`;
  }
  return `连接失败 · ${short}`;
}

/**
 * Minimal live probe via the existing provider.translate path.
 * Uses CURRENT (possibly unsaved) provider settings; does not persist.
 */
export async function testProviderConnection({
  ...
  const cfg = (providerConfig || settings) && typeof (providerConfig || settings) === "object"
    ? (providerConfig || settings)
    : {};
  const missing = missingRequiredField(provider, cfg);
  if (missing) {
    return { ok: false, missingField: missing.label };
  }
  const probe = Array.isArray(texts) && texts.length ? texts.map(String) : [PROVIDER_PROBE_TEXT];
  try {
    const translations = await provider.translate(probe, {
      sourceLang: sourceLang || "en",
      targetLang: targetLang || "zh-CN",
      settings: cfg
    });
    const sample = String(translations?.[0] || "").trim();
    if (!sample) {
      return { ok: false, error: sanitizeProviderError("已连通，但未返回译文", cfg) };
    }
    return { ok: true, translation: sample };
  } catch (err) {
    return {
      ok: false,
      httpStatus: Number(err?.httpStatus) || Number(String(err?.message || "").match(/HTTP\s+(\d{3})/)?.[1]) || undefined,
      error: sanitizeProviderError(err, cfg)
    };
  }
}
