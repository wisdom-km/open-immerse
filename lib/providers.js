...
async function translateWithSkill(texts, ctx, complete) {
  const skillPrompt = resolveTranslatorPrompt(ctx);
...
/**
 * Align numbered model lines to source slots by the printed index.
 * An omitted or empty item stays "" — never collapse and re-zip (that
 * shifts Getting started onto 「设计原则」).
 */
export function parseNumbered(raw, count) {
  ...
  const plain = lines.map((l) => l.replace(/^\s*\d+[\.\)、]\s*/, "").trim()).filter(Boolean);
  for (let i = 0; i < n && i < plain.length; i++) out[i] = plain[i];
  return out;
}
