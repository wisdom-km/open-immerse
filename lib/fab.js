(function (root) {
  const OIFab = {
    ...
    // Scheme B: 停止 only while inflight (incl. two-step polish).
    ...
    fabToggleLabel(state) {
      return state === "translating" ? OIFab.TOGGLE_STOP : OIFab.TOGGLE_TRANSLATE;
    },
    ...
    cornerToPos({
      ...
      const next = OIFab.normalizeCorner(corner);
      const w = Math.max(0, Number(width) || 0);
      const h = Math.max(0, Number(height) || 0);
      const vw = Math.max(0, Number(viewportWidth) || 0);
      const vh = Math.max(0, Number(viewportHeight) || 0);
      const edge = Number.isFinite(Number(inset)) ? Number(inset) : OIFab.SLOT_INSET;
      return OIFab.clampDragPosition({
        left: next === "bottom-left" ? edge : vw - w - edge,
        top: vh - h - edge,
        width: w,
        height: h,
        viewportWidth: vw,
