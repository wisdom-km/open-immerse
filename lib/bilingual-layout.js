(function (root) {
  ...
  const OIBilingual = {
    ...
    bindHost(translation, source) {
      if (!translation || !source) return source || null;
      const bound = hosts.get(translation);
      const forId = OIBilingual.attr(translation, "data-oi-for");
      const srcId = OIBilingual.attr(source, "data-oi-rail-id");
      const srcHash = OIBilingual.attr(source, "data-oi-src-hash");
      const glossHash = OIBilingual.attr(translation, "data-oi-src-hash");
      if (bound && bound !== source) return bound;
      if (forId && srcId && forId !== srcId) return bound || null;
      if (srcHash && glossHash && srcHash !== glossHash) return bound || null;
      hosts.set(translation, source);
      if (srcId) {
        if (typeof translation.setAttribute === "function") translation.setAttribute("data-oi-for", srcId);
        else if (translation.dataset) translation.dataset.oiFor = srcId;
      }
      if (srcHash && !glossHash) {
        if (typeof translation.setAttribute === "function") translation.setAttribute("data-oi-src-hash", srcHash);
        else if (translation.dataset) translation.dataset.oiSrcHash = srcHash;
      }
      return source;
    },

    translationBelongsTo(translation, source) {
      if (!translation || !source) return false;
      const srcHash = OIBilingual.attr(source, "data-oi-src-hash");
      const glossHash = OIBilingual.attr(translation, "data-oi-src-hash");
      if (srcHash && glossHash && srcHash !== glossHash) return false;
      if (hosts.get(translation) === source) return !srcHash || !glossHash || srcHash === glossHash;
      const forId = OIBilingual.attr(translation, "data-oi-for");
      const srcId = OIBilingual.attr(source, "data-oi-rail-id");
      if (forId && srcId) return forId === srcId && (!srcHash || !glossHash || srcHash === glossHash);
      if (forId && hosts.get(translation) && hosts.get(translation) !== source) return false;
      return !forId || !srcId;
    },

    detachStaleGloss(source, liveHash) {
      const expected = String(liveHash || OIBilingual.attr(source, "data-oi-src-hash") || "");
      const raw = [];
      const push = (node) => {
        if (node?.classList?.contains("oi-translation") && !raw.includes(node)) raw.push(node);
      };
      ...
      if (parent && /vue-recycle-scroller__item-view/.test(String(parent.className || ""))) {
        if (typeof parent.querySelectorAll === "function") parent.querySelectorAll(".oi-translation").forEach(push);
      }
      const stale = raw.some((node) => OIBilingual.attr(node, "data-oi-src-hash") !== expected);
      const removed = [];
      (stale ? raw : []).forEach((node) => {
    ...
    nestSideRailInLabel(source, translation) {
      ...
      if (!OIBilingual.translationBelongsTo(translation, source)) return translation;
    ...
    hostForTranslation(node) {
      ...
      const forId = OIBilingual.attr(node, "data-oi-for");
      const doc = node.ownerDocument;
      if (forId && doc?.querySelector) {
        const found = doc.querySelector(`[data-oi-rail-id="${forId}"]`);
    ...
    collectHostTranslations(source) {
      ...
      return out.filter((node) => OIBilingual.translationBelongsTo(node, source));
    },
    ...
    layoutTranslation(translation, source) {
      ...
      const keyed = OIBilingual.hostForTranslation(translation);
      const host =
        source && OIBilingual.translationBelongsTo(translation, source)
          ? source
          : keyed || source;
