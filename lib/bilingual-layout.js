(function (root) {
  ...
  const OIBilingual = {
    ...
    sourceContentWidth(source) {
      if (!source) return 0;
      return OIBilingual.sourceContentBox(source).width
        || OIBilingual.elementBoxWidth(source)
        || OIBilingual.fallbackHostWidth(source);
    },

    /** Content-box width + left (padding/border inset), for clamp + indent. */
    sourceContentBox(source) {
      if (!source) return { width: 0, left: 0 };
      const rect = source.getBoundingClientRect?.() || {};
      const padL = OIBilingual.computedLength(source, "paddingLeft");
      const padR = OIBilingual.computedLength(source, "paddingRight");
      const borderL = OIBilingual.computedLength(source, "borderLeftWidth");
      const boxW = OIBilingual.elementBoxWidth(source) || OIBilingual.fallbackHostWidth(source);
      const client = Number(source.clientWidth) || 0;
      let width = boxW;
      if (padL || padR) {
        const inner = client > 0 ? client - padL - padR : boxW - padL - padR;
        if (inner > 0) width = Math.min(boxW || inner, inner);
      }
      return {
        width: width > 0 ? width : boxW,
        left: (Number(rect.left) || 0) + borderL + padL
      };
    },
    ...
    /**
     * Column clamp is for shrink-wrapped card/grid headings (TRIAD §C.1),
     * not indented body paragraphs whose used width is already the source box.
     */
    ...
    sourceAlignOffset(source, translation) {
      ...
      const parentContentLeft = (Number(parentRect.left) || 0)
        + OIBilingual.computedLength(parent, "paddingLeft")
        + OIBilingual.computedLength(parent, "borderLeftWidth");
      const offset = OIBilingual.sourceContentBox(source).left - parentContentLeft;
      if (!Number.isFinite(offset) || offset <= 0.5) return 0;
      return Math.round(offset * 100) / 100;
    },

    resolveLayout(source, translation) {
      const box = OIBilingual.sourceContentBox(source);
      ...
      const offset = useColumn ? 0 : OIBilingual.sourceAlignOffset(source, translation);
      return { width, offset, useColumn };
    },

    /**
     * Card/grid H3 → column. Body / indented blocks → source content box.
     */
    ...
    applySourceOffset(translation, offset) {
      ...
      if (px > 0) {
        translation.style.marginLeft = `${Math.round(px * 100) / 100}px`;
      } else if (translation.style.marginLeft && /px\s*$/.test(translation.style.marginLeft)) {
    ...
    layoutTranslation(translation, source) {
      ...
      return { extra, width: laid.width, offset: laid.offset, stacked: !OIBilingual.sharesFlexRow(host, keep) };
    }
  };
