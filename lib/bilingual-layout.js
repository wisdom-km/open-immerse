(function (root) {
  ...
  const OIBilingual = {
    ...
    detachStaleGloss(source, liveHash) {
      ...
      if (sib?.classList?.contains("oi-translation")) raw.push(sib);
      if (typeof source?.querySelectorAll === "function") {
        source.querySelectorAll(".oi-translation").forEach((node) => {
          if (!raw.includes(node)) raw.push(node);
        });
      }
      const removed = [];
      raw.forEach((node) => {
        const nodeHash = OIBilingual.attr(node, "data-oi-src-hash");
        if (expected && nodeHash === expected) return;
