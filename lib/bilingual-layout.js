(function (root) {
  ...
  const OIBilingual = {
    ...
    HEADING_MARGIN_TOP_EM: -0.45,
    HEADING_PAD_TOP_EM: 0.32,
    OPTICAL_GAP_MAX_EM: 0.5,
    OPTICAL_GAP_TARGET_EM: 0.35,
    OPTICAL_GAP_MAX_PX: 12,
    OPTICAL_GAP_MIN_PX: 8,
    GAP_CLAMP_STEP_PX: 3,
    GAP_CLAMP_CAP_EM: 3,
    ...
    opticalGapMaxPx(fontSize) {
      ...
      return Math.min(OIBilingual.OPTICAL_GAP_MAX_PX, Math.max(OIBilingual.OPTICAL_GAP_MIN_PX, fs * OIBilingual.OPTICAL_GAP_MAX_EM));
    },

    opticalGapTargetPx(fontSize) {
      const fs = OIBilingual.fontSizePx(fontSize);
      const raw = fs * OIBilingual.OPTICAL_GAP_TARGET_EM;
      const max = OIBilingual.opticalGapMaxPx(fs);
      return Math.min(max, Math.max(OIBilingual.OVERLAP_GAP_MIN, raw));
    },
    ...
    shouldClampOpticalGap(translation, source) {
    ...
    applyOverlapExtra(translation, extraPx) {
      ...
      if (OIBilingual.preferOverlapPadding(translation)) {
        ...
        translation.style.marginTop = `calc(${OIBilingual.BODY_MARGIN_TOP_EM}em + ${extra}px)`;
      }
      return extra;
    },
    ...
    applyGapPull(translation, pullPx) {
      ...
      if (pull <= 0) {
        ...
        if (mt.includes(" - ")) translation.style.marginTop = "";
        return 0;
      }
      const base = OIBilingual.preferOverlapPadding(translation)
        ? OIBilingual.HEADING_MARGIN_TOP_EM
        : OIBilingual.BODY_MARGIN_TOP_EM;
      translation.style.marginTop = `calc(${base}em - ${pull}px)`;
      return pull;
    },

    /**
     * After mount: if source-bottom → translation-top exceeds max optical
     * (~0.35–0.5em / 8–12px), pull margin-top. Overlap bump stays for tight gaps.
     */
    clampOversizedGap(translation, source, opts = {}) {
      ...
      const maxGap = opts.maxGap ?? OIBilingual.opticalGapMaxPx(fontSize);
      const target = opts.targetGap ?? OIBilingual.opticalGapTargetPx(fontSize);
      const step = opts.step ?? OIBilingual.GAP_CLAMP_STEP_PX;
      const cap = opts.capPx ?? OIBilingual.gapClampCapPx(fontSize);
      let pull = 0;
    ...
    layoutTranslation(translation, source) {
      ...
      const pull = keep.classList?.contains("oi-inline") ? 0 : OIBilingual.clampOversizedGap(keep, host);
