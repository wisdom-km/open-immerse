(function (root) {
  ...
  const OIBilingual = {
    ...
    /**
     * §C.1: min(source, column); if source is shrink-wrap tiny, use card/grid column.
     */
    resolveClampWidth(source) {
      ...
      const shrink =
        sourceW < OIBilingual.SHRINK_WRAP_MIN_PX ||
        (parentW > 0 && sourceW < parentW * OIBilingual.SHRINK_WRAP_PARENT_RATIO);
      if (shrink) {
        return columnW || parentW || sourceW;
      }
      const capped = [sourceW, columnW].filter((n) => n > 0);
      return capped.length ? Math.min(...capped) : sourceW;
    },
