...
const PERMISSION_RE = /provided proper attribution/i;

export function isPageChromeText(text) {
  const t = String(text || "").trim();
  if (!t) return true;
  if (PAGE_NUM_RE.test(t)) return true;
  if (ARXIV_RE.test(t)) return true;
...
export function isPageChromeItem(item, page = {}) {
  ...
  const fromBottom = y;
  if (fromTop < h * 0.07 || fromBottom < h * 0.06) {
...
export function extractReadoutBlocks(input, pageSize = {}) {
  ...
  const segmented = segmentPageBlocks(visible);
