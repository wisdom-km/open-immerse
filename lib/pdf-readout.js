...
export function isArxivStampText(text) {
  ...
  // The page stamp carries a version or a category. A bibliography id does not.
  if (/arxiv:\d{4}\.\d+v\d+/i.test(compact)) return true;
  if (/arxiv/i.test(compact) && /\[[a-z]{2}(?:\.[a-z]{2})?\]/i.test(raw)) return true;
  if (/^a\s+r\s+X\s+i\s+v/i.test(raw)) return true;
  return false;
}
...
export function isPageChromeText(text) {
  ...
  if (isArxivStampText(t)) return true;
...
export function translatableReadoutUnits(blocks) {
  return (blocks || [])
    ...
    .filter((block) => {
      ...
      return block.role !== "formula" && block.role !== "figure" && block.role !== "authors";
    });
}
