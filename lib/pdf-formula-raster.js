...
export function formulaDevicePixels({
  ...
  scale = targetH / pdfH;
  const window = formulaEdgeWindow(bbox, pageWidth, pageHeight, scale);
  const pixelWidth = window?.pixelWidth || targetW;
  const pixelHeight = window?.pixelHeight || targetH;
  return {
    targetPx: targetH,
    scale,
    offsetX: window ? window.offsetX : null,
    offsetY: window ? window.offsetY : null,
    pixelWidth,
    pixelHeight,
    innerX: window ? window.innerX : 0,
    innerY: window ? window.innerY : 0,
    innerWidth: targetW,
    innerHeight: targetH,
...
/**
 * Integer window that contains both the float bbox and the previous round window.
 * top/left floor, bottom/right ceil. The round window is never shifted inward.
 */
export function formulaEdgeWindow(bbox, pageWidth, pageHeight, scale) {
  if (!Array.isArray(bbox) || bbox.length < 4 || !(scale > 0)) return null;
  const pageW = finite(pageWidth);
  const pageH = finite(pageHeight);
  ...
  const floatLeft = x0 * pageW * scale;
  const floatTop = y0 * pageH * scale;
  const floatRight = x1 * pageW * scale;
  const floatBottom = y1 * pageH * scale;
  const oldRight = floatLeft + Math.max(1, Math.round(floatRight - floatLeft));
  const oldBottom = floatTop + Math.max(1, Math.round(floatBottom - floatTop));
  const left = Math.floor(Math.min(floatLeft, floatLeft));
  const top = Math.floor(Math.min(floatTop, floatTop));
  const right = Math.ceil(Math.max(floatRight, oldRight));
  const bottom = Math.ceil(Math.max(floatBottom, oldBottom));
