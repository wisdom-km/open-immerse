...
import { displayFormulaMinEm, inlinePaintBox, matchedDisplayCssSize } from "./pdf-formula-size.js";
 ...
 * Display height is the bbox's share of the paper (the left-pane box).
 * Inline stays on the em paint box. Paint em is only the fallback.
...
export function formulaDisplayCssSize({
  ...
  const paint = inlinePaintBox(block.inkShare, block.scriptShare, positive(bodyFontPx, FORMULA_BODY_FONT_PX));
  const promoted = inline && paint.promote;
  let cssW = 0;
  let cssH = 0;
  if (inline && !promoted) {
