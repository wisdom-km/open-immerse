...
export function formulaDevicePixels({
  ...
  return {
    targetPx: pixelHeight,
    scale,
    offsetX: window ? window.offsetX : 0,
    offsetY: window ? window.offsetY : 0,
    pixelWidth,
    pixelHeight,
    cssWidth: pixelWidth / (dpr * zoom),
    cssHeight: pixelHeight / (dpr * zoom),
    devicePixelRatio: dpr,
    mirrorZoom: zoom,
    reusePageRaster: false,
    multiplier: 1
...
/** Inclusive device window for a normalized bbox. Partial edge pixels stay inside. */
function formulaEdgeWindow(bbox, pageWidth, pageHeight, scale) {
  ...
  const sx = Math.floor(x0 * fullW);
  const sy = Math.floor(y0 * fullH);
  const ex = Math.ceil(x1 * fullW);
  const ey = Math.ceil(y1 * fullH);
  if (!(ex > sx) || !(ey > sy)) return null;
  return { offsetX: sx, offsetY: sy, pixelWidth: ex - sx, pixelHeight: ey - sy };
}
