...
/** Right-pane / article nodes: `{ tag, text }[]`. */
export function canExportReadout(nodes) {
  return Array.isArray(nodes) && nodes.some((node) => String(node?.text || "").trim());
}

export function translationExportFilename(basename, ext) {
  const base = String(basename || "").trim() || "PDF";
  const e = String(ext || "md").replace(/^\./, "").toLowerCase();
  return `${base}-译文.${e}`;
}

export function articleBlocksToMarkdown(nodes) {
  const parts = (nodes || [])
    .map((node) => {
      const text = String(node?.text || "").trim();
      if (!text) return "";
      const tag = String(node.tag || node.tagName || "p").toLowerCase();
...
export function wrapExportLine(line, maxChars = 42) {
  const text = String(line || "");
  if (!text) return [""];
  const limit = Math.max(8, Number(maxChars) || 42);
...
export function articleBlocksToPdf(nodes) {
  const lines = [];
  (nodes || []).forEach((node) => {
    const text = String(node?.text || "").trim();
    if (!text) return;
    wrapExportLine(text).forEach((part) => lines.push(part));
    lines.push("");
  });
  return buildSimplePdf(lines.length ? lines : [""]);
}

export function linesToPdf(lines) {
  return buildSimplePdf(Array.isArray(lines) && lines.length ? lines : [""]);
}
