...
/** Slightly generous pageRaster margin so subscripts and parentheses stay inside the crop. */
...
function formulaDraft(items, viewport, host, pad = null) {
  const boxes = items.map((item) => itemBbox(item, viewport));
  const chosen = pad || (host
    ? { x: 0, y: 0 }
    : { x: FORMULA_CROP_PAD.displayX, y: FORMULA_CROP_PAD.displayY });
  return {
    kind: "formula",
    bbox: padBbox(unionBbox(boxes), chosen.x, chosen.y),
    inlineOf: host || null,
    sourceItems: items
  };
}
  ...
  while (index < items.length) {
    ...
    if (span && index === span[0]) {
      ...
      const formula = formulaDraft(raw, viewport, null, {
        x: FORMULA_CROP_PAD.inlineX,
        y: FORMULA_CROP_PAD.inlineY
      });
