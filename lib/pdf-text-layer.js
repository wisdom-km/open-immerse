 ...
 * At crop scale 2 on a 612×792 page, 0.0049 × 612 × 2 ≈ 6 CSS px.
 */
...
/** Hard ceiling. About 6 CSS px at crop scale 2 (612×792), near 1 image pixel per CSS px. */
export const FORMULA_PAD_LIMIT = { x: 0.0049, y: 0.0038 };
...
/**
 * Raster pixels kept outside detected ink. At crop scale 2 these are about CSS px
 * when the readout shows one image pixel per CSS pixel. Display stays looser than
 * inline so equation numbers and script overhang keep a hairline. Both are ≤ ~6.
 */
 ...
 */
export function trimFormulaBboxToInk(bbox, image, { inline = false, protect = false } = {}) {
  if (!Array.isArray(bbox) || bbox.length < 4) return null;
  const original = [clamp01(bbox[0]), clamp01(bbox[1]), clamp01(bbox[2]), clamp01(bbox[3])];
  const width = image?.width | 0;
  const height = image?.height | 0;
  const data = image?.data;
  if (!width || !height || !data || data.length < width * height * 4) return original;
  if (!(original[2] > original[0]) || !(original[3] > original[1])) return original;

  let left = Math.max(0, Math.floor(original[0] * width));
  let top = Math.max(0, Math.floor(original[1] * height));
  let right = Math.min(width, Math.ceil(original[2] * width));
  let bottom = Math.min(height, Math.ceil(original[3] * height));
  if (right - left < 1 || bottom - top < 1) return original;
  ...
  [ix0, ix1] = growCropEdge(ix0, ix1, 24, left, right);
  [iy0, iy1] = growCropEdge(iy0, iy1, 12, top, bottom);
  if (!(ix1 > ix0) || !(iy1 > iy0)) return original;
  return [
    clamp01(ix0 / width),
    clamp01(iy0 / height),
    clamp01(ix1 / width),
...
function displayFormulaGroups(lines, viewport) {
  ...
  for (const anchor of lines) {
    ...
    // Indented relations stay display even when a roman fragment such as "model"
    // or "warmup" sits in the equation. A sentence with function words does not.
    const indentedEquation = anchor.x > pageWidth * 0.24 && anchor.width >= 70 &&
      anchor.width < pageWidth * 0.7 && /=/.test(text) && text.length < 160 &&
      !hasProseFunctionWord(text);
    if (members.has(anchor) || anchor.width < 24 || anchor.width >= pageWidth ||
...
function separateAuthorRows(lines, viewport) {
  ...
  const columns = clusterAuthorColumns(items);
  const authors = columns.length >= 2
    ? columns.map((column) => authorDraftFromColumn(column, viewport)).filter((draft) => draft?.text)
    : [];
...
function clusterAuthorColumns(items) {
  const list = (items || []).filter((item) => String(item?.str || "").trim());
  const lines = [];
  ...
  const seeds = lines[0].items.map((item) => ({
    x: item.x + (Number(item.width) || 0) / 2,
    items: [item]
  }));
  for (const line of lines.slice(1)) {
    for (const item of line.items) {
      const center = item.x + (Number(item.width) || 0) / 2;
      let best = null;
      let bestDist = Infinity;
      for (const seed of seeds) {
        const dist = Math.abs(seed.x - center);
        if (dist < bestDist) {
          bestDist = dist;
          best = seed;
        }
      }
      const limit = Math.max(80, Number(item.width) || 0);
      if (best && bestDist <= limit) best.items.push(item);
      else seeds.push({ x: center, items: [item] });
    }
  }
  return seeds.sort((a, b) => a.x - b.x);
}

function authorCellFromColumn(items) {
  const lines = [];
  ...
  for (const line of lines) {
    const state = { text: "", prev: null };
    [...line.items].sort((a, b) => a.x - b.x).forEach((item) => pushPiece(state, item.str, item));
    const text = state.text.replace(/\s+/g, " ").trim();
    if (!text) continue;
    if (/@/.test(text)) emails.push(text);
    else if (!named) {
      names.push(text);
      named = true;
    } else affiliations.push(text);
  }
  return {
    name: names.join(" ").trim(),
    affiliation: affiliations.join(" ").trim(),
    email: emails.join(" ").trim()
  };
}

function authorDraftFromColumn(column, viewport) {
  const sourceItems = column.items || [];
  const cell = authorCellFromColumn(sourceItems);
  if (!cell.name) return null;
  const authorCell = { name: cell.name };
  if (cell.affiliation) authorCell.affiliation = cell.affiliation;
  if (cell.email) authorCell.email = cell.email;
  return {
    kind: "prose",
    role: "authors",
    text: cell.name,
...
function formulaDraft(items, viewport, { inline = false, obstacles = [] } = {}) {
  ...
  return {
    kind: "formula",
    bbox: contentAwareFormulaBbox(glyphBox, pad, obstacles),
    display: !inline,
    inlineOf: null,
    sourceItems: items,
    glyphBoxes: boxes,
