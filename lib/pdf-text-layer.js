...
function sourceOrderLines(items) {
  ...
  const flush = () => {
    ...
    const state = { text: "", prev: null };
    group.forEach((item) => pushPiece(state, item.str, item));
    lines.push({
      text: state.text.replace(/\s+/g, " ").trim(),
      x: left,
      y: group[0].y,
      ...
      bold: group.some((item) => /bold|black|heavy|semibold/i.test(item.fontName)),
      fontName: group[0].fontName,
      math: group.some(isFormulaItem),
      items: group
    });
    group = [];
  };
  for (const item of items) {
    ...
    if (group.length) {
      ...
      const height = Math.max(8, first.height, item.height);
      const dy = Math.abs(item.y - first.y);
      const reset = item.x < first.x + 20 && last.x + last.width - item.x > 20;
      if (dy > height * 1.15 || (reset && (dy > height * 0.35 || last.hasEOL))) flush();
    }
    group.push(item);
  }
  flush();
...
function lineWithInlineFormulas(line, viewport, nextFormula, formulaByToken) {
  const items = [...(line.items || [])].sort((a, b) => a.x - b.x);
  const spans = inlineFormulaSpans(items);
  const state = { text: "", prev: null };
  ...
  while (index < items.length) {
    ...
    if (span && index === span[0]) {
      ...
      const formula = formulaDraft(raw, viewport, null);
      formula.bbox = padBbox(formula.bbox, 0.002);
...
function proseDrafts(lines, allLines, viewport, formulaByToken) {
  return paragraphsFromLines(lines, allLines, { preserveLineHyphens: true }).flatMap((block) => {
    ...
    entries.forEach((entry) => { entry.formula.inlineOf = host; });
...
function toProtocol(drafts, pageNumber, trust) {
  ...
  const blocks = drafts.map((draft) => {
    ...
    if (draft.kind === "formula") {
      const block = { id: draft.id, sourceId: draft.sourceId, label: "formula", bbox: draft.bbox };
...
export function textLayerToBlocks({ items, viewport, images, page } = {}) {
  ...
  const lines = rawLines.map(lineWithScripts);
  const drafts = [];
  let prose = [];
  let formulaSerial = 0;
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    if (grouped) {
      if (grouped.anchor === rawLines[lineIndex]) {
        flushProse();
        drafts.push(formulaDraft(grouped.group.flatMap((part) => expandedItems(part.items || [])), view, null));
      }
      continue;
    }
    ...
    if (formulaLine && parts.length) {
      flushProse();
      drafts.push(formulaDraft(expandedItems(parts), view, null));
      continue;
    }
    prose.push(lineWithInlineFormulas(line, view, nextFormula, formulaByToken));
  }
  flushProse();
