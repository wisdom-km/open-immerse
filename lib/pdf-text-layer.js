...
/**
 * Soft-merge band for adjacent display rows, as a fraction of line height.
 * Inclusive at lo, exclusive at hi: a gap of 1.6 line-heights stays two blocks.
 */
...
/** Baseline gap divided by the taller line's em. Tests lock lo/hi against this. */
export function displayLineGapRatio(upper, lower) {
...
/** "where / with / for" plus math continues a display. A prose where-clause does not. */
function isRelationHead(line) {
  const text = String(line?.text || "");
  const match = relationHeadMatch(text);
  if (!match) return false;
  const rest = text.trim().slice(match[0].length);
  if (hasBlockingProse(rest) || hasProseFunctionWord(rest)) return false;
...
function canJoinDisplayLine(line, nearest, group, ordered, pageWidth) {
  ...
  const ratio = displayLineGapRatio(nearest, line);
  if (equationNumberOnGroup(line, group) && ratio < DISPLAY_CROSSLINE_GAP.hi) return true;
  if (ratio >= DISPLAY_CROSSLINE_GAP.hi) return false;
  ...
  if (hasBlockingProse(line.text) && !isRelationHead(line)) return false;
  if (mathFragment(line, group)) return true;
  if (isRelationHead(line)) return relationAligns(nearest, line, pageWidth);
...
function absorbDisplayGroup(group, ordered, pageWidth) {
  let grew = true;
  while (grew) {
    grew = false;
    for (const line of ordered) {
      ...
      if (!canJoinDisplayLine(line, nearest, group, ordered, pageWidth)) continue;
      group.push(line);
      grew = true;
    }
  }
...
function displayFormulaGroups(lines, viewport) {
  ...
  for (const anchor of ordered) {
    ...
    const group = [anchor];
    absorbDisplayGroup(group, ordered, pageWidth);
...
function formulaDraft(items, viewport, { inline = false, obstacles = [] } = {}) {
  ...
  return {
    kind: "formula",
    bbox: contentAwareFormulaBbox(inkBox, { x: pad.x, y: pad.y }, obstacles),
    display: !inline,
    inlineOf: null,
    sourceItems: items,
    glyphBoxes: boxes,
...
function isStrongMathItem(item, items, index, metrics) {
  ...
  // h_{t−1} folded in a text face stays prose. A math-face identifier can still anchor.
...
export function textLayerToBlocks({ items, viewport, images, page } = {}) {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    if (grouped && grouped.anchor !== rawLines[lineIndex] && grouped.emitted) continue;
    ...
    if (grouped && grouped.anchor === rawLines[lineIndex] && !continuation) {
      flushProse();
