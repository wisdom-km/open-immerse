...
/**
 * How far formula ink may extend past a glyph box before the neighbor mask
 * wins. On the Attention p4-b9 2× crop (em 13.95px) the 23 dark pixels the
 * old mask erased sit at most 14 steps of 8-connected growth from glyph-box
 * ink. 1.15em is 17px there: those 23 pixels plus a 3px margin.
 */
export const FORMULA_MASK_KEEP_EM = 1.15;

/**
 * Paint masked pixels paper-white. Seeds are dark pixels inside formula glyph
 * boxes only. Ink may grow a short chebyshev distance from those seeds, and
 * only inside the glyph boxes' x-range, so a radical tip survives while a
 * neighbor line that merely touches does not.
 */
export function blankFormulaMask(image, { maskBoxes = [], glyphBoxes = [] } = {}) {
  ...
  const boxes = (glyphBoxes || []).filter((box) => Array.isArray(box) && box.length >= 4);
  let em = 0;
  let x0 = width;
  let x1 = 0;
  for (const box of boxes) {
    em = Math.max(em, (Number(box[3]) - Number(box[1])) * height);
    x0 = Math.min(x0, Math.floor(Number(box[0]) * width));
    x1 = Math.max(x1, Math.ceil(Number(box[2]) * width));
  }
  ...
  const seed = (x, y) => {
    ...
    if (dist[slot] !== -1) return;
    if (isPaperPixel(data, slot * 4)) return;
    dist[slot] = 0;
    queue.push(slot);
  };
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      if (!boxes.some((box) => pointInBox(nx, ny, box))) continue;
      seed(x, y);
    }
  }
  // BFS: the first visit is the minimum chebyshev distance. A stack would
  // lock a long path in and drop tip pixels that only the short path reaches.
