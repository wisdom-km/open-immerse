...
/**
 * When the scan's bottom row still holds formula ink, follow that stroke
 * downward at most this far. The next line's letter top can stop it sooner.
 */
export const FORMULA_INK_DESCENT_EM = 0.35;
...
function rowHasFormulaInk(data, width, y, left, right, height, maskBoxes, glyphBoxes) {
  const row = y * width * 4;
  const ny = (y + 0.5) / height;
...
export function measureFormulaCrop(bbox, image, options = {}) {
  ...
  // A dark bottom row is a clipped stroke. Follow it down to the first blank
  // row, and no farther than 0.35em or the next line's letter top. A blank
  // bottom row leaves the crop on the ebd674f path.
  if (glyphBoxes.length && maxY === bottom - 1 &&
      rowHasFormulaInk(data, width, bottom - 1, left, right, height, maskBoxes, glyphBoxes)) {
    const em = Math.max(...glyphBoxes.map((box) => Number(box[3]) - Number(box[1])));
    let cap = scan[3] + FORMULA_INK_DESCENT_EM * Math.max(0, em);
    const explicitTop = Number(options?.letterTop);
    if (Number.isFinite(explicitTop)) cap = Math.min(cap, explicitTop);
    for (const box of options?.maskBoxes || []) {
      if (!Array.isArray(box) || box.length < 4) continue;
      const topEdge = Number(box[1]);
      if (topEdge >= scan[3] - 1e-6) cap = Math.min(cap, topEdge);
    }
    const limit = Math.min(height, Math.floor(cap * height));
    for (let y = bottom; y < limit; y += 1) {
      if (!rowHasFormulaInk(data, width, y, left, right, height, maskBoxes, glyphBoxes)) break;
      maxY = y;
      bottom = y + 1;
    }
  }
