...
/**
 * How far a radical or large operator may extend past its own glyph box
 * before the neighbor mask wins. On the Attention p4-b9 2× crop the radical
 * box is 13.95px tall and the tip is 14 steps of 8-connected growth away.
 * 1.15em is 17px there: those steps plus a 3px margin.
 */
export const FORMULA_MASK_KEEP_EM = 1.15;

/**
 * Glyphs whose ink is drawn outside the pdf.js font box: radicals and the
 * large operators. Ordinary letters, digits, and other symbols are not seeds.
 * A CMEX (or similar) face is included only when its name says radical, or
 * when the text item is already one of these characters.
 */
const FORMULA_MASK_SEED_CHAR = /[\u220F-\u2211\u221A-\u221C\u222B-\u2233\u22C0-\u22C3\u2A00-\u2A1E]/;

export function isFormulaMaskSeedItem(item = {}) {
  const str = String(item.str || item.text || "").replace(/\s+/g, "");
  if (str && [...str].every((ch) => FORMULA_MASK_SEED_CHAR.test(ch))) return true;
  const font = `${item.fontRealName || ""} ${item.fontName || ""} ${item.glyphName || ""}`;
  return /radical/i.test(font);
}

/**
 * Paint masked pixels paper-white. Only radical and large-operator glyph
 * boxes seed the keep-flood, and each one grows only inside its own x-range.
 */
export function blankFormulaMask(image, { maskBoxes = [], glyphBoxes = [], seedBoxes = [] } = {}) {
  const width = image?.width | 0;
  const height = image?.height | 0;
  const data = image?.data;
  if (!width || !height || !data || !maskBoxes.length) return image;
  const seeds = (seedBoxes || []).filter((box) => Array.isArray(box) && box.length >= 4);
  const keep = new Uint8Array(width * height);
  for (const box of seeds) {
    const x0 = Math.max(0, Math.floor(Number(box[0]) * width));
    const x1 = Math.min(width, Math.ceil(Number(box[2]) * width));
    if (x1 <= x0) continue;
    const em = Math.max(0, (Number(box[3]) - Number(box[1])) * height);
    ...
    for (let y = 0; y < height; y += 1) {
      ...
      for (let x = x0; x < x1; x += 1) {
        if (!pointInBox((x + 0.5) / width, ny, box)) continue;
        const slot = y * width + x;
        if (isPaperPixel(data, slot * 4)) continue;
        dist[slot] = 0;
        keep[slot] = 1;
        queue.push(slot);
      }
    ...
    while (head < queue.length) {
      ...
      head += 1;
      const x = slot % width;
      const y = (slot - x) / width;
      const next = dist[slot] + 1;
      ...
      for (let dy = -1; dy <= 1; dy += 1) {
        for (let dx = -1; dx <= 1; dx += 1) {
          ...
          const nslot = ny * width + nx;
          if (dist[nslot] !== -1) continue;
          if (isPaperPixel(data, nslot * 4)) continue;
          dist[nslot] = next;
          keep[nslot] = 1;
          queue.push(nslot);
        }
...
function formulaSeedBoxes(items, viewport) {
  const boxes = [];
  for (const item of expandedItems(items)) {
    if (!isFormulaMaskSeedItem(item)) continue;
...
function formulaDraft(items, viewport, { inline = false, obstacles = [] } = {}) {
  ...
  return {
    ...
    sourceItems: items,
    glyphBoxes: boxes,
    seedBoxes: formulaSeedBoxes(items, viewport),
...
function toProtocol(drafts, pageNumber, trust) {
  ...
  const blocks = drafts.map((draft) => {
    ...
    if (draft.kind === "formula") {
      ...
      if (draft.seedBoxes?.length) block.seedBoxes = draft.seedBoxes;
