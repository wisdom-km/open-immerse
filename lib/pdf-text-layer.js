...
export function measureFormulaCrop(bbox, image, options = {}) {
  ...
  // Main's 2× PNG scans the whole content-aware box. The glyph window stops
  // at the font box, so a radical tip below that edge never enters the redraw.
  // Keep the tight window unless that lower strip actually has formula ink.
  if (glyphBoxes.length && original[3] > scan[3]) {
    const fullBottom = Math.min(height, Math.ceil(original[3] * height));
    let lower = -1;
    for (let y = bottom; y < fullBottom; y += 1) {
      const row = y * width * 4;
      const ny = (y + 0.5) / height;
      for (let x = left; x < right; x += 1) {
        const index = row + x * 4;
