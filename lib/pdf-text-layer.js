...
/** Neighbor font boxes miss descenders. Grow them by this share of the em, plus one page pixel. */
export const NEIGHBOR_INK_EM = 0.25;
/** Cap-height and descent, as shares of the font box above and below the baseline. */
export const FORMULA_CORE_CAP = 0.72;
export const FORMULA_CORE_DESCENT = 0.25;
/** A bar this wide is a fraction rule or radical vinculum, not a neighbor fragment. */
export const FORMULA_BAR_WIDTH = 0.5;
...
/**
 * Font box grown by 0.25em plus one page pixel on the vertical sides.
 * Boxes taller than a text line are left alone so a figure region does not swallow the formula.
 */
export function expandNeighborInkBox(box, pageHeight) {
  if (!Array.isArray(box) || box.length < 4) return box;
  const em = box[3] - box[1];
  if (!(em > 0) || em > 0.08) return box.slice(0, 4);
  const height = Math.max(1, Number(pageHeight) || 1);
  const pad = em * NEIGHBOR_INK_EM + 1 / height;
  return [box[0], clamp01(box[1] - pad), box[2], clamp01(box[3] + pad)];
}

function glyphCoreRect(box) {
  const em = box[3] - box[1];
  if (!(em > 0)) return null;
  const baseline = box[3];
  return [box[0], baseline - FORMULA_CORE_CAP * em, box[2], baseline + FORMULA_CORE_DESCENT * em];
}

/**
 * Whiten ink that is only a neighbor fragment.
 * A component stays when it hits a formula glyph's core (cap-height through descent),
 * or when it spans more than half the formula width (fraction rule, radical bar).
 * A component glued to the crop edge stays only when it also has ink inside a core.
 */
export function dropDetachedFormulaInk(image, { glyphBoxes = [] } = {}) {
  const width = image?.width | 0;
  const height = image?.height | 0;
  const data = image?.data;
  const cores = (glyphBoxes || []).map(glyphCoreRect).filter(Boolean);
  if (!width || !height || !data || !cores.length) return image;
  let formulaLeft = 1;
  let formulaRight = 0;
  for (const box of glyphBoxes || []) {
    if (!Array.isArray(box) || box.length < 4) continue;
    formulaLeft = Math.min(formulaLeft, box[0]);
    formulaRight = Math.max(formulaRight, box[2]);
  }
  const formulaWidth = Math.max(1, Math.round((formulaRight - formulaLeft) * width));
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (labels[start] || isPaperPixel(data, start * 4)) continue;
      nextLabel += 1;
      const comp = { minX: x, minY: y, maxX: x, maxY: y, pixels: [], deepCore: false, label: nextLabel };
      ...
      while (stack.length) {
        ...
        comp.pixels.push(current);
        if (cx < comp.minX) comp.minX = cx;
        if (cy < comp.minY) comp.minY = cy;
        if (cx > comp.maxX) comp.maxX = cx;
        if (cy > comp.maxY) comp.maxY = cy;
        const nx = (cx + 0.5) / width;
        const ny = (cy + 0.5) / height;
        if (cores.some((core) => pointInBox(nx, ny, core))) {
          comp.inCore = true;
          if (cy > 1 && cy < height - 2) comp.deepCore = true;
        }
        for (let dy = -1; dy <= 1; dy += 1) {
          for (let dx = -1; dx <= 1; dx += 1) {
            ...
            const neighbor = indexOf(xx, yy);
            if (labels[neighbor] || isPaperPixel(data, neighbor * 4)) continue;
  ...
  for (const comp of components) {
    const wide = comp.maxX - comp.minX + 1 > formulaWidth * FORMULA_BAR_WIDTH;
    const touchesEdge = comp.minY === 0 || comp.maxY === height - 1;
    if (wide || comp.deepCore || (comp.inCore && !touchesEdge)) continue;
...
function glyphScanLimit(bbox, glyphBoxes, pad = FORMULA_GLYPH_SCAN_PAD) {
...
export function measureFormulaCrop(bbox, image, options = {}) {
  ...
  const scan = glyphBoxes.length ? glyphScanLimit(original, glyphBoxes, options?.scanPad) : original;
...
function formulaDraft(items, viewport, { inline = false, obstacles = [] } = {}) {
  ...
  return {
    ...
    sourceItems: items,
    glyphBoxes: boxes,
    maskBoxes: intersectingMaskBoxes(
      (obstacles || []).map((box) => expandNeighborInkBox(box, viewport?.height)),
      bbox
    ),
...
function preparedTextLines({ items, viewport, images } = {}) {
