...
/**
 * Extra scan below the glyph union. pdf.js font boxes end at the baseline, so a
 * radical tip hangs past that edge. The top pad stays FORMULA_GLYPH_SCAN_PAD.y
 * so the previous line is not pulled in. Still clamped to the formula bbox.
 */
export const FORMULA_GLYPH_DESCENT_PAD = 0.006;
...
function glyphScanLimit(bbox, glyphBoxes, pad = FORMULA_GLYPH_SCAN_PAD) {
  ...
  const padBottom = Math.max(padY, Number(pad?.bottom) || 0, FORMULA_GLYPH_DESCENT_PAD);
  return [
    Math.max(bbox[0], union[0] - padX),
    Math.max(bbox[1], union[1] - padY),
    Math.min(bbox[2], union[2] + padX),
    Math.min(bbox[3], union[3] + padBottom)
  ];
}
