 ...
 */
export function measureFormulaCrop(bbox, image, { inline = false, protect = false } = {}) {
...
/** Measured ink / right-pane body font-size. Hard floor is 1; target band is 1.05–1.15. */
export const INLINE_INK_TARGET = 1.1;
...
export function inlineCropBoxEm(inkShare, target = INLINE_INK_TARGET) {
  ...
  let share = Number(inkShare);
  if (!(share > 0.2) || share > 1 || !Number.isFinite(share)) share = nominal;
