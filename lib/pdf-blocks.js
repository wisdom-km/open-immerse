/** blocks-1 protocol, crop math, and block cleanup. No layout HTTP. */

import { cropCanvasToDataUrl } from "./pdf-mirror.js";
import { PDF_PAPER_PAD_X } from "./pdf-paper.js";
import { plaintextRelationsIn } from "./pdf-plaintext-formula.js";

export const PROTOCOL = "blocks-1";
export const CROP_SCALE = 2;

export const LABELS = [
  "title",
  "heading",
  "text",
  "caption",
  "formula",
  "figure",
  "table",
  "header",
  "footer"
];

/** Engine ids. Wiring them to a service is a later phase; this module does not fetch. */
export const LAYOUT_MODES = [
  "text-layer", // 划区不可用时的兜底
  "local-ocr", // 测试期本机智谱 GLM-OCR
  "cloud-ocr" // 上线 OCR API
];

export const TEXT_LAYER_BAD_RATIO = 0.05;
export const TEXT_LAYER_PRINTABLE_RATIO = 0.6;

/** Placeholder token ⟦fN⟧. Fresh regex per call; the global one keeps lastIndex. */
export const PLACEHOLDER_RE = /⟦f(\d+)⟧/g;

export const OCR_PAGE_HINT =
  "本页文字层不可用，正文来自识别结果，可能有误差。公式、图、表仍是原页截图。";

/** Appended to both PDF translation requests. Webpage batches do not send this. */
export const PDF_PROMPT_ADDENDUM = [
  "The segment may contain tokens ⟦f1⟧ ⟦f2⟧ and so on. Copy every such token into the translation unchanged, with the same spelling, count, and order.",
  "Keep numbers, citation markers, and variable names exactly as written.",
  "Do not add, remove, or rewrite mathematical symbols. If a symbol appears outside a token, copy it unchanged.",
  "Faithfulness to the source outranks smoother wording."
].join("\n");

const TRANSLATABLE_LABELS = new Set(["title", "heading", "text", "caption"]);

const VISUAL_LABELS = new Set(["formula", "figure", "table"]);
const VISUAL_DROP_KEYS = ["latex", "content", "html", "md", "text"];
const VISUAL_ALT = { formula: "公式", figure: "图", table: "表" };

export function isVisualBlock(block) {
  return VISUAL_LABELS.has(block?.label);
}

/** Alt text is a kind name. Recognized formula letters never become the alt. */
export function visualAlt(label) {
  if (label === "figure" || label === "image") return "图";
  if (label === "table") return "表";
  return "公式";
}

export function isTranslatableBlock(block) {
  if (!block || block.skipTranslate === true) return false;
  if (!TRANSLATABLE_LABELS.has(block.label)) return false;
  return String(block.text || "").trim() !== "";
}

/** Natural-language blocks only. Formula, figure, and table stay out of the batch. */
export function translatableBlocks(blocks) {
  return (blocks || []).filter(isTranslatableBlock);
}

/** Write translations back by block id. Image fields on the source blocks stay put. */
export function applyBlockTranslations(blocks, results = []) {
  const byId = new Map();
  const ordered = [];
  for (const row of results || []) {
    const result = { translation: String(row?.translation || ""), status: row?.translationStatus || row?.status || "" };
    if (row?.id) byId.set(row.id, result);
    else ordered.push(result);
  }
  let cursor = 0;
  return (blocks || []).map((block) => {
    if (!isTranslatableBlock(block)) return { ...block };
    let result = { translation: "", status: "" };
    if (byId.has(block.id)) result = byId.get(block.id);
    else if (cursor < ordered.length) result = ordered[cursor++];
    return {
      ...block,
      translation: result.translation,
      ...(result.status ? { translationStatus: result.status } : {})
    };
  });
}

function citationMarks(text) {
  return [...String(text || "").matchAll(/\[\s*\d+(?:\s*,\s*\d+)*\s*\]/g)]
    .map((match) => match[0].replace(/\s+/g, ""));
}

/**
 * Old library rows still carry ⟦fN⟧ for relations that are now Unicode.
 * Replace only those extra tokens with the source relations, in order.
 * A real missing token or a citation mismatch is left untouched.
 * Returns "" when there is nothing to remap.
 */
export function remapRetiredPlaintextPlaceholders(block = {}) {
  const translation = String(block.translation || "");
  if (!translation || !/⟦f\d+⟧/.test(translation)) return "";
  const source = String(block.text || "");
  const sourceTokens = placeholderTokens(source).map((entry) => entry.token);
  const targetTokens = placeholderTokens(translation).map((entry) => entry.token);
  if (JSON.stringify(sourceTokens) === JSON.stringify(targetTokens)) return "";
  let cursor = 0;
  const extraAt = [];
  targetTokens.forEach((token, index) => {
    if (cursor < sourceTokens.length && token === sourceTokens[cursor]) cursor += 1;
    else extraAt.push(index);
  });
  if (cursor !== sourceTokens.length || !extraAt.length) return "";
  const relations = plaintextRelationsIn(source);
  if (relations.length < extraAt.length) return "";
  const extra = new Set(extraAt);
  let seen = 0;
  let used = 0;
  const remapped = translation.replace(/⟦f\d+⟧/g, (token) => {
    const index = seen;
    seen += 1;
    if (!extra.has(index)) return token;
    const relation = relations[used];
    used += 1;
    return relation;
  });
  if (used !== extraAt.length) return "";
  if (JSON.stringify(citationMarks(block.sourceText || source)) !== JSON.stringify(citationMarks(remapped))) {
    return "";
  }
  if (/\[\s*(?:,\s*)*\]/.test(remapped)) return "";
  return remapped;
}

/** Identifier already written as Unicode, including h_t and h_{t−1}. Fresh each call. */
function unicodeMathFragments(text) {
  const pattern = /[A-Za-z\u0370-\u03FF](?:_\{[A-Za-z0-9−–+\-]+\}|_[A-Za-z0-9−–+\-]+)/g;
  return [...String(text || "").matchAll(pattern)].map((match) => ({
    text: match[0],
    index: match.index
  }));
}

/**
 * Live text gained ⟦fN⟧ after an identifier was cropped, while the stored
 * translation still has that identifier as Unicode. Put the extra tokens
 * back, in order, so integrity can pass without a retranslate.
 * Returns "" when the Unicode fragments cannot be lined up safely.
 */
export function remapLiveExtraPlaceholders(block = {}) {
  const translation = String(block.translation || "");
  const source = String(block.text || "");
  if (!translation || !/⟦f\d+⟧/.test(source)) return "";
  const sourceTokens = placeholderTokens(source).map((entry) => entry.token);
  const targetTokens = placeholderTokens(translation).map((entry) => entry.token);
  if (JSON.stringify(sourceTokens) === JSON.stringify(targetTokens)) return "";
  let tokenCursor = 0;
  const extraTokens = [];
  for (const token of sourceTokens) {
    if (tokenCursor < targetTokens.length && token === targetTokens[tokenCursor]) tokenCursor += 1;
    else extraTokens.push(token);
  }
  if (tokenCursor !== targetTokens.length || !extraTokens.length) return "";
  const sourceMath = unicodeMathFragments(source);
  const targetMath = unicodeMathFragments(translation);
  let mathCursor = 0;
  const slots = [];
  for (const frag of targetMath) {
    if (mathCursor < sourceMath.length && frag.text === sourceMath[mathCursor].text) mathCursor += 1;
    else slots.push(frag);
  }
  if (mathCursor !== sourceMath.length || slots.length !== extraTokens.length) return "";
  let out = "";
  let last = 0;
  slots.forEach((slot, index) => {
    out += translation.slice(last, slot.index) + extraTokens[index];
    last = slot.index + slot.text.length;
  });
  out += translation.slice(last);
  if (JSON.stringify(placeholderTokens(out).map((entry) => entry.token)) !== JSON.stringify(sourceTokens)) {
    return "";
  }
  if (JSON.stringify(citationMarks(block.sourceText || source)) !== JSON.stringify(citationMarks(out))) {
    return "";
  }
  if (/\[\s*(?:,\s*)*\]/.test(out)) return "";
  return out;
}

function blockForIntegrity(block = {}) {
  const remapped = remapRetiredPlaintextPlaceholders(block) || remapLiveExtraPlaceholders(block);
  if (!remapped) return block;
  return { ...block, translation: remapped };
}

/** A translation with missing protected marks cannot safely place its formula crops. */
export function blockTranslationIntegrity(block = {}) {
  const view = blockForIntegrity(block);
  const translation = String(view.translation || "");
  if (!translation) return { valid: true, reason: "" };
  const sourceTokens = placeholderTokens(view.text).map((entry) => entry.token);
  const targetTokens = placeholderTokens(translation).map((entry) => entry.token);
  if (JSON.stringify(sourceTokens) !== JSON.stringify(targetTokens)) {
    return { valid: false, reason: "formula-placeholder-mismatch" };
  }
  if (JSON.stringify(citationMarks(view.sourceText || view.text)) !== JSON.stringify(citationMarks(translation)) ||
      /\[\s*(?:,\s*)*\]/.test(translation)) {
    return { valid: false, reason: "citation-mismatch" };
  }
  return { valid: true, reason: "" };
}

const PLACEHOLDER_TOKEN_RE = /^⟦f\d+⟧$/;

/** Formula source strings written into sourceText in place of ⟦fN⟧. */
function formulaSourceByToken(block = {}) {
  const text = String(block.text || "");
  const sourceText = String(block.sourceText || "");
  const map = new Map();
  if (!text || !sourceText || text === sourceText) return map;
  const parts = text.split(/(⟦f\d+⟧)/);
  let at = 0;
  for (let index = 0; index < parts.length; index += 1) {
    const part = parts[index];
    if (PLACEHOLDER_TOKEN_RE.test(part)) {
      let end = sourceText.length;
      for (let next = index + 1; next < parts.length; next += 1) {
        const literal = parts[next];
        if (!literal || PLACEHOLDER_TOKEN_RE.test(literal)) continue;
        const found = sourceText.indexOf(literal, at);
        if (found >= 0) end = found;
        break;
      }
      map.set(part, sourceText.slice(at, end));
      at = end;
    } else if (part) {
      if (sourceText.startsWith(part, at)) at += part.length;
      else {
        const found = sourceText.indexOf(part, at);
        if (found >= 0) at = found + part.length;
      }
    }
  }
  return map;
}

function cleanFormulaLabel(value) {
  const text = String(value || "").trim();
  if (!text || PLACEHOLDER_TOKEN_RE.test(text) || /⟦f\d+⟧/.test(text)) return "";
  return text;
}

/** Unicode or source-text stand-ins for tokens the translation still spells as ⟦fN⟧. */
function spareFormulaLabels(block, translation) {
  const labels = [];
  const seen = new Set();
  const push = (value) => {
    const text = cleanFormulaLabel(value);
    if (!text || seen.has(text) || String(translation || "").includes(text)) return;
    seen.add(text);
    labels.push(text);
  };
  for (const label of formulaSourceByToken(block).values()) push(label);
  const source = String(block.sourceText || block.text || "");
  const targetFrags = unicodeMathFragments(translation);
  let cursor = 0;
  for (const frag of unicodeMathFragments(source)) {
    if (cursor < targetFrags.length && frag.text === targetFrags[cursor].text) cursor += 1;
    else push(frag.text);
  }
  return labels;
}

/** Split a sentence into text and original-page formula crops. */
export function blockRenderPieces(block, blocks = []) {
  const view = blockForIntegrity(block);
  const byId = new Map((blocks || []).map((item) => [item.id, item]));
  const placeholders = view?.placeholders || [];
  const known = new Map(placeholders.map((entry) => [entry.token, entry.blockId]));
  const integrity = blockTranslationIntegrity(view);
  const translation = String(view?.translation || "");
  const body = translation || String(view?.text || "");
  const pieces = [];
  const placed = new Set();
  const spareLabels = spareFormulaLabels(view, body);
  let spareAt = 0;
  const pushImage = (token) => {
    const blockId = known.get(token);
    if (!blockId || placed.has(token)) return false;
    placed.add(token);
    const formula = byId.get(blockId);
    pieces.push({
      type: "image",
      blockId,
      alt: "公式",
      src: formula?.imageUrl || ""
    });
    return true;
  };
  const nextLiveToken = () => {
    for (const entry of placeholderTokens(view.text)) {
      if (placed.has(entry.token) || !known.has(entry.token)) continue;
      const formula = byId.get(known.get(entry.token));
      if (formula?.imageUrl) return entry.token;
    }
    return "";
  };
  body.split(/(⟦f\d+⟧)/).forEach((part) => {
    if (!PLACEHOLDER_TOKEN_RE.test(part)) {
      if (part) pieces.push({ type: "text", text: part });
      return;
    }
    if (known.has(part)) {
      pushImage(part);
      return;
    }
    // A stored ⟦fN⟧ that no longer matches this page: use the live crop,
    // then the formula's own text. Never leave the token in the sentence.
    const borrowed = nextLiveToken();
    if (borrowed && pushImage(borrowed)) return;
    const label = spareLabels[spareAt] || "";
    if (label) {
      spareAt += 1;
      pieces.push({ type: "text", text: label });
    }
  });
  // A translation that still has slots can take the crops it missed at the end.
  // A translation with no slots stays plain text: do not dump the English source.
  if (translation && !integrity.valid && placeholderTokens(translation).length) {
    for (const entry of placeholderTokens(view?.text)) {
      if (placed.has(entry.token)) continue;
      pushImage(entry.token);
    }
  }
  return pieces;
}

export function visualBlockMarkdown({ alt = "公式", src = "" } = {}) {
  const label = String(alt || "公式").trim() || "公式";
  const href = String(src || "").trim();
  return href ? `![${label}](${href})` : "见图";
}

export function rasterCropRect(bbox, pixelWidth, pixelHeight) {
  const [x0, y0, x1, y1] = bbox;
  const sx = Math.round(x0 * pixelWidth);
  const sy = Math.round(y0 * pixelHeight);
  const sw = Math.max(1, Math.round((x1 - x0) * pixelWidth));
  const sh = Math.max(1, Math.round((y1 - y0) * pixelHeight));
  return { sx, sy, sw, sh };
}

export function bboxToPercentRect(bbox) {
  const [x0, y0, x1, y1] = bbox;
  return { left: x0 * 100, top: y0 * 100, width: (x1 - x0) * 100, height: (y1 - y0) * 100 };
}

/** Crop a pageRaster canvas. Viewer calls this name so it does not name the mirror crop helper. */
export function cropBlockImage(canvas, bbox) {
  if (!canvas || !Array.isArray(bbox)) return "";
  const rect = rasterCropRect(bbox, canvas.width, canvas.height);
  if (rect.sw < 24 || rect.sh < 12) return "";
  return cropCanvasToDataUrl(canvas, bboxToPercentRect(bbox));
}

/** Same crop window as cropBlockImage, still a canvas so a formula mask can paint before PNG. */
export function cropBlockCanvas(canvas, bbox) {
  if (!canvas || !Array.isArray(bbox) || typeof canvas.getContext !== "function") return null;
  const rect = rasterCropRect(bbox, canvas.width, canvas.height);
  if (rect.sw < 24 || rect.sh < 12) return null;
  try {
    const owner = canvas.ownerDocument || globalThis.document;
    const out = owner?.createElement?.("canvas");
    if (!out) return null;
    out.width = rect.sw;
    out.height = rect.sh;
    const ctx = out.getContext("2d");
    if (!ctx) return null;
    ctx.drawImage(canvas, rect.sx, rect.sy, rect.sw, rect.sh, 0, 0, rect.sw, rect.sh);
    return out;
  } catch {
    return null;
  }
}

export function textLayerTrust(items) {
  const text = (items || []).map((item) => item.str || "").join("");
  if (!text.trim()) return { trusted: false, reason: "empty" };
  const bad = (text.match(/[\uFFFD\uE000-\uF8FF]/g) || []).length;
  const printable = (text.match(/[\u0020-\u007E\u00A0-\u024F\u0370-\u03FF\u4E00-\u9FFF]/g) || []).length;
  if (bad / text.length > TEXT_LAYER_BAD_RATIO || printable / text.length < TEXT_LAYER_PRINTABLE_RATIO) {
    return { trusted: false, reason: "garbled" };
  }
  return { trusted: true, reason: "text-layer" };
}

export function placeholderTokens(text) {
  return [...String(text || "").matchAll(/⟦f(\d+)⟧/g)].map((hit) => ({
    token: hit[0],
    n: Number(hit[1])
  }));
}

/** Visual blocks must not carry recognized letters onto the page or into translation. */
export function normalizeIncomingBlock(block) {
  if (!block || typeof block !== "object") return block;
  const next = { ...block };
  if (!isVisualBlock(next)) return next;
  for (const key of VISUAL_DROP_KEYS) delete next[key];
  return next;
}

/** Drop page chrome. Keep the remaining array order (reading order). */
export function preparePageBlocks(page = {}) {
  const blocks = [];
  for (const block of page.blocks || []) {
    const next = normalizeIncomingBlock(block);
    if (!next || next.label === "header" || next.label === "footer") continue;
    blocks.push(next);
  }
  return {
    ...page,
    protocol: page.protocol || PROTOCOL,
    blocks
  };
}

/**
 * Display-crop width as a fraction of the page (bbox x1 − x0).
 * This is not a percent of the padded readout column. Applying it there
 * shrinks the formula a second time; use displayContentColumnFraction.
 */
export function displayCropColumnFraction(block = {}) {
  if (block.label !== "formula" || block.inlineOf || block.display === false) return null;
  if (!Array.isArray(block.bbox) || block.bbox.length < 4) return null;
  const fraction = Number(block.bbox[2]) - Number(block.bbox[0]);
  if (!(fraction > 0)) return null;
  return Math.min(1, fraction);
}

/** Share of the paper width left after PDF_PAPER_PAD_X on both sides. */
export function contentColumnShare(padX = PDF_PAPER_PAD_X) {
  const pad = Number(padX);
  if (!Number.isFinite(pad) || !(pad > 0)) return 1;
  const share = 1 - 2 * pad;
  if (!(share > 0) || share > 1) return 1;
  return share;
}

/**
 * Hard floor for display height / right-pane body font-size: ≥ 1.6×.
 * Readable band is 1.6–2.0×. A natural ~3× after the width fix still passes
 * when it stays ≤ 5×. The old 2.5–3.5× band is not a fail gate.
 * The crop box is taller than the ink (#62 keeps a paper margin), so the
 * CSS min-height sits above the floor: 2em × ~0.8 ink share ≈ 1.6.
 * It only grows a crop that is still under the floor. It does not stretch
 * every formula up to the column.
 */
export const DISPLAY_INK_FLOOR = 1.6;
export const DISPLAY_CROP_MIN_HEIGHT_EM = 2;
/** Exclusive display prefers measured ink ≥ 1.4× body. Hard floor stays ≥ 1.0×; the 1.6 band stays the #63 strong expectation. */
export const DISPLAY_INK_PREFER = 1.4;
/**
 * Paint fallback only. The formula CSS height is the left-pane bbox.
 * This stops a low ink share from stretching the crop past 2.5× body.
 */
export const DISPLAY_BODY_HARD_MAX = 2.5;

/**
 * Box em that keeps measured ink at the display prefer band.
 * Unknown share keeps the #63 2em box. A short crop whose pad would leave
 * ink under 1.4× grows only this floor — column width stays first.
 */
export function displayInkMinEm(inkShare, boxEm = DISPLAY_CROP_MIN_HEIGHT_EM) {
  const box = Number(boxEm) > 0 && Number.isFinite(Number(boxEm)) ? Number(boxEm) : DISPLAY_CROP_MIN_HEIGHT_EM;
  const share = Number(inkShare);
  if (!(share > 0.2) || share > 1 || !Number.isFinite(share)) return box;
  if (box * share + 1e-9 >= DISPLAY_INK_PREFER) return box;
  return Math.round((DISPLAY_INK_PREFER / share) * 10000) / 10000;
}

/**
 * Width of a display crop as a fraction of the padded readout column.
 * pageFraction / (1 − 2×pad) undoes the second shrink. When the left page
 * is wider than the paper, scale toward it. Never above the column (1).
 */
export function displayContentColumnFraction(pageFraction, options = {}) {
  const fraction = Number(pageFraction);
  if (!(fraction > 0) || !Number.isFinite(fraction)) return null;
  let width = fraction / contentColumnShare(options.padX ?? PDF_PAPER_PAD_X);
  const left = Number(options.leftWidth);
  const paper = Number(options.paperWidth);
  if (paper > 0 && left > paper) width *= left / paper;
  if (!(width > 0) || !Number.isFinite(width)) return null;
  return Math.min(1, width);
}

function cssNumber(value) {
  const n = Math.round(Number(value) * 10000) / 10000;
  return String(n);
}

/**
 * CSS width for one display crop.
 * A′ percent of the content column, times left page / paper when the paper
 * is narrower, clamped to the column. If that box would still sit under the
 * 1.6× floor, grow only enough for min-height — not straight to 100%.
 */
export function displayCropWidthCss(pageFraction, bboxHeightFraction, minEm = DISPLAY_CROP_MIN_HEIGHT_EM) {
  const fraction = Number(pageFraction);
  if (!(fraction > 0) || !Number.isFinite(fraction)) return "";
  const em = Number(minEm) > 0 && Number.isFinite(Number(minEm)) ? Number(minEm) : DISPLAY_CROP_MIN_HEIGHT_EM;
  const pct = cssNumber((fraction / contentColumnShare()) * 100);
  const boosted = `calc(var(--oi-pdf-left-w, var(--oi-pdf-paper-w, 1px)) / var(--oi-pdf-paper-w, 1px) * ${pct}%)`;
  const height = Number(bboxHeightFraction);
  if (!(height > 0) || !Number.isFinite(height)) return `min(100%, ${boosted})`;
  const floor = `calc(${cssNumber(em)}em * var(--oi-pdf-paper-w, 0px) * ${cssNumber(fraction)} / (var(--oi-pdf-paper-h-base, 1px) * ${cssNumber(height)}))`;
  return `min(100%, max(${boosted}, ${floor}))`;
}

/**
 * Display crop width with the column cap removed.
 * F3-S2: one width, height:auto. F3-S4: if this exceeds the column, the
 * formula scrollport scrolls; the image is not shrunk below the ink floor.
 */
export function displayFormulaWidthCss(pageFraction, bboxHeightFraction, minEm = DISPLAY_CROP_MIN_HEIGHT_EM) {
  const fraction = Number(pageFraction);
  if (!(fraction > 0) || !Number.isFinite(fraction)) return "";
  const requested = Number(minEm) > 0 && Number.isFinite(Number(minEm)) ? Number(minEm) : DISPLAY_CROP_MIN_HEIGHT_EM;
  const em = Math.min(requested, DISPLAY_BODY_HARD_MAX);
  const pct = cssNumber((fraction / contentColumnShare()) * 100);
  const boosted = `calc(var(--oi-pdf-left-w, var(--oi-pdf-paper-w, 1px)) / var(--oi-pdf-paper-w, 1px) * ${pct}%)`;
  const height = Number(bboxHeightFraction);
  if (!(height > 0) || !Number.isFinite(height)) return boosted;
  const floor = `calc(${cssNumber(em)}em * var(--oi-pdf-paper-w, 0px) * ${cssNumber(fraction)} / (var(--oi-pdf-paper-h-base, 1px) * ${cssNumber(height)}))`;
  return `max(${boosted}, ${floor})`;
}

/** Right-pane plan for one blocks-1 item. Images are crops; no LaTeX render. */
export function blockReadoutPlan(block = {}) {
  const label = block.label;
  if (label === "title") return { tag: "h1", className: "oi-pdf-h1", text: block.text || "" };
  if (label === "heading") return { tag: "h2", className: "oi-pdf-h2", text: block.text || "" };
  if (label === "formula") {
    const display = block.display !== false && !block.inlineOf;
    return display
      ? {
          tag: "figure",
          className: "oi-pdf-display-math",
          image: true,
          imageClass: "oi-pdf-math-crop",
          role: "formula-display",
          alt: VISUAL_ALT.formula,
          src: block.imageUrl || ""
        }
      : {
          tag: "span",
          className: "oi-pdf-inline-math",
          image: true,
          imageClass: "oi-pdf-math-crop",
          role: "formula-inline",
          alt: VISUAL_ALT.formula,
          src: block.imageUrl || ""
        };
  }
  if (label === "figure" || label === "table") {
    return {
      tag: "figure",
      className: "oi-pdf-figure",
      image: true,
      imageClass: "oi-pdf-asset-crop",
      alt: VISUAL_ALT[label] || "",
      src: block.imageUrl || ""
    };
  }
  if (label === "caption") {
    return {
      tag: "figcaption",
      className: "oi-pdf-caption",
      text: block.text || "",
      role: "caption"
    };
  }
  if (isVisualBlock(block)) {
    return {
      tag: "p",
      className: "oi-pdf-p",
      image: true,
      alt: VISUAL_ALT[label] || "",
      src: block.imageUrl || ""
    };
  }
  return {
    tag: "p",
    className: "oi-pdf-p",
    text: block.text || "",
    role: block.presentation === "byline" ? "authors" : ""
  };
}
