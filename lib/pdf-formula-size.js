...
/**
 * F09-class inlines often have no scriptShare, so the paint box never grows
 * and the subscript stays under 7px. A missing or sub-0.28 share uses this
 * conservative font-box fraction (inside the 0.28–0.35 band).
 */
export const SCRIPT_SHARE_LOW = 0.28;
export const SCRIPT_SHARE_DEFAULT = 0.32;
...
/**
 * Missing or very small script shares do not describe a subscript.
 * Callers that already have a share at or above 0.28 keep it.
 */
export function effectiveScriptShare(scriptShare) {
  const raw = Number(scriptShare);
  if (!(raw >= SCRIPT_SHARE_LOW) || raw > 1) return SCRIPT_SHARE_DEFAULT;
  return raw;
}
...
export function displayFormulaMinEm(inkShare, scriptShare, bodyFs = FORMULA_BODY_FS_PX) {
  ...
  const raw = Number(scriptShare);
  const share = raw > 0 && raw <= 1 ? raw : SCRIPT_SHARE_DEFAULT;
  const script = scriptMinEm(share, bodyFs, SCRIPT_INK_TARGET_PX);
