 ...
 * toward the 8px paint target. Over ~2.2em the formula becomes display.
 */
export function inlinePaintBox(inkShare, scriptShare, bodyFs = FORMULA_BODY_FS_PX) {
  ...
  const scriptBox = scriptMinEm(scriptShare, bodyFs, SCRIPT_INK_TARGET_PX);
...
export function displayFormulaMinEm(inkShare, scriptShare, bodyFs = FORMULA_BODY_FS_PX) {
  ...
  const script = scriptMinEm(scriptShare, bodyFs, SCRIPT_INK_TARGET_PX);
