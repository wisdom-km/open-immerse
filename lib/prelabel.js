...
export const PRELABEL_VERSION = "m1-2026-09-25";
export const SCRIPT_ATTACHED_CONFIDENCE = 0.55;
export const SCRIPT_BASE_CONFIDENCE = 0.55;
export const MATH_RUN_CONFIDENCE = 0.5;
const LOW_UNIT_CONFIDENCE = 0.55;
const HUGE_INLINE_MEMBERS = 60;
...
function looksLikeProse(glyph) {
  if (!glyph || isKeptText(glyph)) return false;
  const text = textOf(glyph);
...
function canHostScript(glyph) {
  const text = textOf(glyph);
  if (!text || isKeptText(glyph)) return false;
...
function isGlueGlyph(glyph) {
  if (!glyph || isKeptText(glyph) || glyph.rule === "eq-number-candidate") return false;
  const text = textOf(glyph);
...
function isFootnoteSymbol(text) {
...
function labelGlyph(glyph, line, index, lineInfo) {
  ...
  if (isFootnoteSymbol(trimmed)) return { label: "text", confidence: 0.84, rule: "citation-or-footnote" };
...
function labelPath(element, formulaBoxes, pageWidth, pageHeight) {
  ...
  const longRule = (height <= 2.8 && width >= 40) || (width <= 2.8 && height >= 28);
  if (longRule) return { label: "other", confidence: 0.9, rule: "rule-line" };
  if (area > pageArea * 0.02) return { label: "other", confidence: 0.88, rule: "large-path" };
  const fractionBar = height <= 2.4 && width >= 8 && width < 40;
  let near = 0;
  if (fractionBar) {
  ...
  if (fractionBar && near > 0) return { label: "formula", confidence: 0.8, rule: "fraction-bar" };
...
function isKeptText(glyph) {
  return glyph?.rule === "citation-or-footnote" || glyph?.rule === "table-note";
}
...
function markScript(glyph) {
  ...
  glyph.label = "formula";
  glyph.equationNumber = false;
  if (glyph.rule === "script-offset") {
    glyph.confidence = 0.82;
    return;
  }
  glyph.confidence = SCRIPT_ATTACHED_CONFIDENCE;
  glyph.rule = "script-attached";
}

function markScriptBase(glyph) {
  if (glyph.rule === "math-font" || glyph.rule === "math-unicode" || glyph.rule === "script-offset") return;
  glyph.label = "formula";
  glyph.confidence = SCRIPT_BASE_CONFIDENCE;
  glyph.rule = "script-base";
  glyph.equationNumber = false;
}

function markMathRun(glyph) {
  if (glyph.label === "formula" && (glyph.rule === "math-font" || glyph.rule === "math-unicode" || glyph.rule === "script-offset")) return;
  glyph.label = "formula";
  glyph.confidence = MATH_RUN_CONFIDENCE;
  glyph.rule = "math-run";
  glyph.equationNumber = false;
}

function markTableNote(glyph) {
  glyph.label = "text";
  glyph.confidence = 0.84;
  glyph.rule = "table-note";
  glyph.equationNumber = false;
  glyph.unitId = null;
  glyph.unitType = null;
}
...
export function attachScriptsAndRuns(lines) {
  ...
  for (let index = 0; index < sorted.length; index += 1) {
    ...
    if (!isMarkerGlyph(glyph) || isKeptText(glyph)) continue;
    let start = index;
    let end = index;
  ...
  for (let index = 0; index < sorted.length; index += 1) {
    const glyph = sorted[index];
    if (isKeptText(glyph)) continue;
    const text = textOf(glyph);
    if (!/^[abc*]$/.test(text)) continue;
    const beforeNote = adjacentTouching(sorted, index, glyph, -1);
    const afterNote = adjacentTouching(sorted, index, glyph, 1);
    const gapLeft = beforeNote ? horizontalGap(beforeNote, glyph) : Infinity;
    const atStart = !beforeNote || gapLeft > 12;
    if (!atStart) continue;
    const hosted = beforeNote
      && canHostScript(beforeNote)
      && gapLeft <= Math.max(3.2, heightOf(beforeNote) * 0.5)
      && (scriptAgainst(glyph, beforeNote).raised || scriptAgainst(glyph, beforeNote).lowered);
    if (hosted) continue;
    const afterText = textOf(afterNote);
    if (!(afterNote && (looksLikeProse(afterNote) || afterText.length >= 8))) continue;
    markTableNote(glyph);
  }
  for (let index = 0; index < sorted.length; index += 1) {
    const glyph = sorted[index];
    ...
    while (grew) {
      ...
      for (let index = 0; index < glyphs.length; index += 1) {
        ...
        for (const otherIndex of [index - 1, index + 1]) {
          ...
          if (!other || isKeptText(other) || other.rule === "eq-number-candidate") continue;
...
function anchorY(element) {
  const baseline = Number(element.baseline);
  if (Number.isFinite(baseline)) return baseline;
  return ((element.bbox?.[1] || 0) + (element.bbox?.[3] || 0)) / 2;
}

function sameTextLine(a, b) {
  const shift = Math.abs(anchorY(a) - anchorY(b));
  const height = Math.max(heightOf(a), heightOf(b));
  return shift <= Math.max(4, height * 0.92);
}

function sameCluster(a, b, pageWidth) {
  if (!sameTextLine(a, b)) return false;
  const gapX = Math.max(0, Math.max(a.bbox[0], b.bbox[0]) - Math.min(a.bbox[2], b.bbox[2]));
  const height = Math.max(heightOf(a), heightOf(b));
  if (a.equationNumber || b.equationNumber) return gapX < pageWidth * 0.55;
  if (gapX > pageWidth * 0.08) return false;
  return gapX <= Math.max(14, height * 1.25);
}
...
function attachEquationNumbers(elements, pageWidth) {
  ...
  for (const element of elements) {
    ...
    const near = formulas.some((formula) => sameTextLine(formula, element));
    if (!near) continue;
...
function buildUnits(elements, pageWidth, links) {
  ...
  for (let i = 0; i < formulas.length; i += 1) {
    for (let j = i + 1; j < formulas.length; j += 1) {
      const linked = links?.same(formulas[i], formulas[j]) && sameTextLine(formulas[i], formulas[j]);
      if (!sameCluster(formulas[i], formulas[j], pageWidth) && !linked) continue;
  ...
  const units = [];
  function linePieces(members) {
    const pieces = [];
    const sorted = [...members].sort((a, b) => anchorY(a) - anchorY(b) || a.bbox[0] - b.bbox[0]);
    for (const member of sorted) {
      const piece = pieces.find((entry) => entry.some((item) => sameTextLine(item, member)));
      if (!piece) pieces.push([member]);
      else piece.push(member);
    }
    return pieces;
  }
  function emitUnit(members) {
    ...
    if (type === "inline" && members.length >= HUGE_INLINE_MEMBERS) {
      for (const member of members) {
        member.confidence = Math.min(Number(member.confidence) || LOW_UNIT_CONFIDENCE, LOW_UNIT_CONFIDENCE);
      }
    }
  ...
  for (const members of groups.values()) {
    const pieces = linePieces(members);
    const split = pieces.length > 1;
    for (const piece of pieces) {
      if (split) {
        for (const member of piece) {
          member.confidence = Math.min(Number(member.confidence) || LOW_UNIT_CONFIDENCE, LOW_UNIT_CONFIDENCE);
        }
      }
      emitUnit(piece);
