...
/**
 * Same-frame scroll + scrollend: do not drop the driver until the scroll rAF has flushed.
 * noteEnd() returns "defer" while a sync is pending, and finish() returns "release" once.
 */
export function createScrollSyncGate() {
  let pending = 0;
  let endQueued = false;
