 ...
 *
 * Quality lives in DEFAULT_LLM_PROMPT. `guardZhBusinessSense` and
 * `guardZhTitleCalques` are safety nets (主教 fusion / known title calques),
 * not the quality strategy.
 *
 * Optional second pass: top-level `twoStepPolish` (boolean; settings default true).
 * When false/undefined on a raw object: single-pass Skill only.
 * When true: LLM adapters run a faithful draft, then a 达雅 polish.
 * Ordinary language conversion (especially EN → modern Chinese).
 * NOT classical Chinese, NOT 文言文, NOT 《诗经》, NOT literary multi-pass.
 * Non-empty custom `settings.prompt` skips the polish pass (single shot).
 */
...
export const TRANSLATION_SKILL_CONTRACT = [
  ...
  "Optional two-step (twoStepPolish=true): step1 faithful draft, step2 natural fluent polish.",
  "Two-step is ordinary language conversion (e.g. EN→modern ZH), not 文言文, not 《诗经》, not literary multi-pass.",
  "Titles: tight verb-led Chinese; hard-ban hollow taught-us / about-what-content calques.",
  "Never 主教 for business owners. Numbered one-per-line output.",
  "guardZhBusinessSense and guardZhTitleCalques are safety nets, not this Skill.",
  "Settings default twoStepPolish=true (better quality, more tokens). Custom prompt skips polish (single-shot)."
].join(" ");
...
- Titles: prefer tight, verb-led Chinese faithful to "taught us about". FORBID these heading calques: 「让我们了解到」「让我们了解到的」「关于…哪些内容」「哪些关于…的内容」. Never render "What X taught us about Y" as 「X让我们了解到了关于Y的哪些内容」 or 「X让我们了解到了哪些关于Y的内容」.
...
When the target is Chinese, titles and headings must stay tight and verb-led. FORBID these calques: 「让我们了解到」「让我们了解到的」「关于…哪些内容」「哪些关于…的内容」. If the draft uses them, rewrite into concise verb-led Chinese faithful to "taught us about" — do not invent meaning.
Return ONLY the final translations, one per line, prefixed with the same numbers. No explanations.`;

/** True when a per-provider / ctx custom system prompt is set. */
export function hasCustomTranslatorPrompt(settings = {}) {
  return String(settings?.prompt || "").trim() !== "";
}

/**
 * Top-level `twoStepPolish` (settings default true; this helper is true only when
 * the flag is explicitly true). Custom prompt skips the polish pass.
 * Webpage and documents share this flag via the same translate() path.
 */
export function isTwoStepPolish(settings = {}) {
  if (!settings || settings.twoStepPolish !== true) return false;
  if (hasCustomTranslatorPrompt(settings)) return false;
  return true;
}
...
/**
 * Build one LLM turn. Custom `settings.prompt` replaces the Skill and skips polish
 * (`isTwoStepPolish` is then false). Two-step only wraps the shared default Skill.
 */
