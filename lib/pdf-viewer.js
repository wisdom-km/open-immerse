...
export const ZOOM_CHIP_STORAGE_KEY = "oi-pdf-zoom-pos";
export const ZOOM_CHIP_INSET = 12;
export const ZOOM_CHIP_GUTTER_FALLBACK = 16;
...
export function zoomChipRightGutter({ paneRight, pagesRight, scrollbarWidth = 0 } = {}) {
  const pane = Number(paneRight);
  const pages = Number(pagesRight);
  const sb = Math.max(0, Number(scrollbarWidth) || 0);
...
export function clampZoomChipPos({
  ...
  const minL = edge;
  const minT = edge;
  const maxL = Math.max(minL, pw - w - edge);
