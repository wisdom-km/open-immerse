...
function linesToParagraphs(lines, sharedStats) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const heading = isHeadingLine(line, next, stats);
    const nextHeading = next ? isHeadingLine(next, lines[i + 2], stats) : false;
    buf.push(line);
    if (!next) {
