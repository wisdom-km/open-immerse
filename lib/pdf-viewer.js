...
function detectItemColumnSplit(items) {
  ...
  const starts = [...new Set(items.map((item) => Math.round(item.x)))].sort((a, b) => a - b);
  let bestGap = 0;
  let splitX = null;
  for (let i = 1; i < starts.length; i++) {
    ...
    if (rel > 0.22 && rel < 0.78 && gap > bestGap) {
      bestGap = gap;
      splitX = mid;
    }
  }
  if (splitX != null && bestGap > Math.max(16, pageWidth * 0.06)) return splitX;
  return null;
}
...
function linesToParagraphs(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const largeGap = gap > Math.max(medianH * 1.55, line.height * 1.55);
