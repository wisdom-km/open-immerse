...
import { formulaExportMarkdown } from "./pdf-latex.js";
...
export const PDF_COPY = {
  ...
  noTextLayer: "本页没有文字层。",
  noTextLayerHint: "本页没有文字层，无法镜像版式。扫描件翻译将在后续版本支持。",
  mirrorCaption: "版式镜像",
  viewMirror: "版式",
  viewReadout: "通读",
  mirrorHint: "按原页位置排列译文。公式写入 LaTeX 并用 KaTeX 渲染；图为原页裁剪。导出 MD 含可粘贴 $...$ / $$...$$。",
...
export function collectReadoutExportNodes(root) {
  ...
  return [...root.querySelectorAll("h1, h2, p")]
    .map((el) => {
      const latex = String(el.dataset?.latex || "").trim();
      const display = el.dataset?.mathDisplay === "1" || el.dataset?.mathDisplay === "true";
      const formula = el.dataset?.role === "formula" || el.dataset?.kind === "math" || Boolean(latex);
      const text = formula
