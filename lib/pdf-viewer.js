...
/** One article part: a heading or a body paragraph. Chinese-only in the right pane. */
export function translationBlock({ original, translation, page, filename, role } = {}) {
  return {
    ...
    role: role === "heading" ? "heading" : "paragraph"
  };
}

/** Map a translated block to an article node (heading vs body). */
export function articleNodeSpec(block = {}) {
  const role = block.role === "heading" ? "heading" : "paragraph";
  return {
    tag: role === "heading" ? "h2" : "p",
    role,
    text: String(block.translation || "").trim()
  };
}
    ...
    .map((item) => {
      if (item && typeof item === "object") {
        return {
          ...
          role: item.role === "heading" ? "heading" : "paragraph"
        };
      }
...
function linesToParagraphs(lines) {
  ...
  const stats = { medianH, medianW, medianLead };
  const paras = [];
  let buf = [];
  const flush = (role) => {
    if (!buf.length) return;
    const text = joinHyphenated(buf.map((line) => line.text));
    if (text.length > 1) paras.push({ text, role: role === "heading" ? "heading" : "paragraph" });
    buf = [];
  };
  for (let i = 0; i < lines.length; i++) {
    ...
    const heading = isHeadingLine(line, prev, stats);
    const nextHeading = next ? isHeadingLine(next, line, stats) : false;
    buf.push(line);
    if (!next) {
      flush(heading ? "heading" : "paragraph");
      break;
    }
    const gap = line.y - next.y;
    const largeGap = gap > medianLead * 1.45;
    const shortEnd = !heading && line.width < medianW * 0.72 && /[.!?:]$/.test(line.text);
    const sameHeadingBand = heading && nextHeading && Math.abs(line.height - next.height) <= Math.max(1, medianH * 0.2);
    const keepHeading = sameHeadingBand && !largeGap;
    if (keepHeading) continue;
    if (largeGap || shortEnd || heading || nextHeading) flush(heading ? "heading" : "paragraph");
  }
  return paras;
}
...
function isHeadingLine(line, prev, stats) {
  ...
  if (line.height > medianH * 1.28) return true;
  const short = line.text.length <= 48 && line.width < medianW * 0.55;
  const words = line.text.split(/\s+/).filter(Boolean);
  const fewWords = words.length <= 8;
  const endsSentence = /[.!?]$/.test(line.text);
  if (!short || !fewWords || endsSentence) return false;
  const gapBefore = prev ? prev.y - line.y : Infinity;
  const isolated = !prev || gapBefore > medianLead * 1.35;
  const slightlyLarger = line.height >= medianH * 1.12;
  return isolated || slightlyLarger || (short && fewWords);
}
