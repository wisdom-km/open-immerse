...
export function buildMirrorLayout(input, pageSize = {}) {
  ...
  const lines = clusterMirrorLines(items);
  const boxes = linesToMirrorBoxes(lines, pageWidth, pageHeight);
  const textRects = boxes.map((box) => box.rect);
  const formulaVisuals = boxes
    .filter((box) => box.kind === "formula")
    .map((box) => ({ kind: "formula", rect: inflateRect(box.rect, 2) }));
  const figureVisuals = findUncoveredRegions(textRects, pageWidth, pageHeight).map((rect) => ({
    kind: "figure",
    rect
  }));
  const visuals = mergeVisuals(formulaVisuals, figureVisuals, pageWidth, pageHeight);
  return {
    kind: "mirror",
    pageWidth,
    pageHeight,
...
export function toMirrorItem(box, { page, translation, imageUrl } = {}) {
  ...
  return {
    ...
    kind: box?.kind || "text",
    imageUrl: imageUrl || box?.imageUrl || ""
  };
}
...
function clusterMirrorLines(items) {
  ...
  const flush = () => {
    ...
    splitLineByGaps(bucket).forEach((part) => {
...
function splitLineByGaps(items) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    // Author grids leave ~1.5–3em gutters; do not scale the cut by the full row width.
    if (gap > Math.max(14, em * 1.55)) {
...
function linesToMirrorBoxes(lines, pageWidth, pageHeight) {
  ...
  while (remaining.length) {
    ...
    groups.push({
      text,
      role: seedKind === "formula" ? "paragraph" : role,
      kind: seedKind,
      rect: inflateRect(
        {
          left,
