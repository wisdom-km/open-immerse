...
import { formulaDisplayMode, recoverFormulaLatex, unwrapLatex, wrapLatexMarkdown } from "./pdf-latex.js";
...
export function buildMirrorLayout(input, pageSize = {}) {
  ...
  const formulaVisuals = boxes
    .filter((box) => box.kind === "formula" && shouldRenderFormulaCrop(box))
    .map((box) => ({
      kind: "formula",
...
/** A: recovered LaTeX → KaTeX. B: crop. C: 「公式」. Never OCR. */
...
export function readingOrderMirrorItems(layout, translations = []) {
  ...
  const formulas = (layout.boxes || [])
    ...
    .map((box) => {
      ...
      if (plan.mode !== "katex" || !plan.latex) return null;
      return {
        original: box.text,
...
export { formulaDisplayMode, recoverFormulaLatex, unwrapLatex, wrapLatexMarkdown };
