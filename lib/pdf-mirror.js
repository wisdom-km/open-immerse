...
export const PDF_MIRROR_COPY = {
  ...
  viewMirror: "版式",
  viewReadout: "通读",
  hint: "按原页位置排列译文。公式与图为原页裁剪，不编造 TeX。导出 MD/PDF 仍为纯文本。",
...
export function looksLikeFormulaItem(item = {}) {
  ...
  if (/[A-Za-z]{4,}/.test(str) && !/[=∑∫√]/.test(str)) return false;
...
export function buildMirrorLayout(input, pageSize = {}) {
  ...
  boxes.forEach((box) => {
    if (box.kind === "formula") box.render = formulaRenderPlan(box);
  });
  const textRects = boxes.map((box) => box.rect);
  const formulaVisuals = boxes
    .filter((box) => box.kind === "formula")
    .map((box) => ({
      kind: "formula",
      rect: inflateRect(box.rect, 2),
      sourceText: box.text,
      render: formulaRenderPlan(box)
    }));
...
export function translatableMirrorUnits(layout) {
  return (layout?.translatable || layout?.boxes || [])
    .filter((box) => box.kind !== "formula" && String(box.text || box.original || "").trim())
    .map((box) => ({
...
export function looksLikeAuthorCell(box, pageWidth) {
  if (!box || box.kind === "formula") return false;
...
/** A: extractable Unicode/text. B: crop. C: 「公式」. Never invent TeX. */
export function formulaRenderPlan(box = {}) {
  const text = String(box.text || box.str || box.sourceText || "").trim();
  if (text && text.length <= 80 && !looksLikeGarbageGlyphs(text)) {
    return { mode: "unicode", text: unicodeMathify(text), alt: text };
  }
  return { mode: "crop", text: "", alt: PDF_MIRROR_COPY.formulaFallback };
}

/** Skip page-crop when Unicode (A) already has a glyph string. */
export function shouldRenderFormulaCrop(vis) {
  if (!vis || vis.kind !== "formula") return false;
  const plan = vis.render || formulaRenderPlan(vis);
  return !(plan.mode === "unicode" && plan.text);
}
...
function looksLikeGarbageGlyphs(text) {
