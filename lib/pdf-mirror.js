...
/** pdf.js text item ids. They are not PostScript font names. */
const INTERNAL_FONT_ID = /^g_d\d+_f\d+$/i;

/** Embedded subset tag, six capitals, then the real font name. */
const SUBSET_PREFIX = /^[A-Z]{6}\+/;

/**
 * Computer Modern and other math faces. Roman text faces (CMR, Nimbus, Times)
 * are intentionally absent so body prose is not classified as a formula.
 * Longer Computer Modern stems are listed before the shorter ones.
 */
const MATH_FONT_NAME = new RegExp([
  "CM(?:BSY|MIB|BX|MI|SY|EX)\\d*",
  "MS(?:AM|BM)\\d*",
  "rsfs\\d*",
  "eufm\\d*",
  "MTMI\\d*",
...
const BODY_FONT_NAME = /Nimbus|Times|Helvetica|Arial|Courier|Calibri|Georgia|Liberation|TeXGyre|CMR\d/i;

/**
 * Greek, operators, relations, arrows, letterlike symbols, and the
 * mathematical alphanumeric block. The last block is outside the BMP, so it
 * uses a braced escape; a four-digit \u would swallow ordinary letters.
 */
const MATH_UNICODE = /[\u0370-\u03FF\u1F00-\u1FFF\u2100-\u214F\u2190-\u21FF\u2200-\u22FF\u27C0-\u27EF\u27F0-\u27FF\u2900-\u297F\u2A00-\u2AFF\u{1D400}-\u{1D7FF}]/u;

export function stripFontSubsetPrefix(name) {
  return String(name || "").replace(SUBSET_PREFIX, "").trim();
}

/** Real PostScript name when the viewer resolved it; otherwise a caller-supplied face name. */
export function fontNameForFormula(item = {}) {
  const real = stripFontSubsetPrefix(item.fontRealName || "");
  if (real && !INTERNAL_FONT_ID.test(real)) return real;
  const raw = stripFontSubsetPrefix(item.fontName || item.font || "");
  if (!raw || INTERNAL_FONT_ID.test(raw)) return "";
  return raw;
}

export function isMathFontName(name) {
  const font = stripFontSubsetPrefix(name);
  if (!font || INTERNAL_FONT_ID.test(font)) return false;
  if (BODY_FONT_NAME.test(font) && !/Math/i.test(font)) return false;
  return MATH_FONT_NAME.test(font);
}

export function hasMathUnicode(text) {
  return MATH_UNICODE.test(String(text || ""));
}

/**
 * Read page.commonObjs after getTextContent. A missing or unfinished font
 * leaves the item unchanged so detection falls back to glyphs.
 */
export function attachFontRealNames(items, commonObjs) {
  const list = Array.isArray(items) ? items : [];
  const cache = new Map();
  for (const item of list) {
    if (!item || typeof item !== "object") continue;
    const id = String(item.fontName || "");
    if (!id) continue;
    if (!cache.has(id)) {
      let resolved = "";
      try {
        const ready = typeof commonObjs?.has === "function" ? commonObjs.has(id) : false;
        const font = ready && typeof commonObjs?.get === "function" ? commonObjs.get(id) : null;
        resolved = stripFontSubsetPrefix(font?.name || "");
        if (!resolved || INTERNAL_FONT_ID.test(resolved)) resolved = "";
        cache.set(id, {
          name: resolved,
          ascent: Number.isFinite(Number(font?.ascent)) ? Number(font.ascent) : null,
          descent: Number.isFinite(Number(font?.descent)) ? Number(font.descent) : null
        });
      } catch {
        cache.set(id, { name: "", ascent: null, descent: null });
      }
    }
    const fontInfo = cache.get(id);
    if (fontInfo?.name) item.fontRealName = fontInfo.name;
    if (fontInfo && item.ascent == null && fontInfo.ascent != null) item.ascent = fontInfo.ascent;
    if (fontInfo && item.descent == null && fontInfo.descent != null) item.descent = fontInfo.descent;
  }
  return list;
}

export function looksLikeFormulaItem(item = {}) {
  if (isMathFontName(fontNameForFormula(item))) return true;
