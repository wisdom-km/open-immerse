...
const MIRROR_TEXT_ROLES = new Set(["title", "authors", "heading", "caption", "paragraph", "body", "margin"]);
...
export function boxesOverlapX(a, b, slop = 1) {
  ...
  return Math.min(ga.left + ga.width, gb.left + gb.width) - Math.max(ga.left, gb.left) > slop;
}

export function boxesOverlapY(a, b, slop = 1) {
  const ga = mirrorBoxGeom(a);
  const gb = mirrorBoxGeom(b);
  return Math.min(ga.top + ga.height, gb.top + gb.height) - Math.max(ga.top, gb.top) > slop;
}
...
function cloneLayoutBox(box, index) {
  ...
  return {
    ...
    role: box.role || "",
    dir: box.dir || (box.vertical ? "vertical" : ""),
...
/** Drop ghost / double-painted boxes: same text at nearly the same place. */
export function dedupeMirrorInkBoxes(boxes = []) {
  ...
  for (const box of list) {
    ...
    const hit = text
      ? kept.find((other) => {
          const t2 = String(other.text || "").replace(/\s+/g, " ").trim();
          if (!t2 || text !== t2) return false;
          const near = Math.abs(box.top - other.top) < 14 && Math.abs(box.left - other.left) < 18;
          return near || inkOverlapRatio(box, other) > 0.45;
        })
      : null;
    if (hit) {
      if (boxArea(box) > boxArea(hit)) {
        kept[kept.indexOf(hit)] = box;
        dropped.push(hit);
      } else dropped.push(box);
      continue;
    }
    kept.push(box);
  }
 ...
 * Same-row cells (no X overlap) and vertical margin strips stay put.
 */
export function resolveVerticalMirrorCollisions(boxes = [], { gap = MIRROR_COLLISION_GAP } = {}) {
  ...
  for (let i = 0; i < list.length; i++) {
    if (list[i].dir === "vertical" || list[i].role === "margin") continue;
    let push = 0;
    for (let j = 0; j < i; j++) {
      if (list[j].dir === "vertical" || list[j].role === "margin") continue;
...
/** Left/right margin strips keep a horizontal gutter so body cannot bleed into them. */
export function reserveMarginMeta(boxes = [], pageWidth, pageHeight, { gap = 6 } = {}) {
  const pw = Math.max(1, Number(pageWidth) || 1);
  const ph = Math.max(1, Number(pageHeight) || 1);
  const list = (boxes || []).map((box, index) => cloneLayoutBox(box, index));
  const metas = list.filter((box) => isMarginMirrorBox({ ...box, rect: box }, pw, ph));
  if (!metas.length) return list;
  const pad = Math.max(4, Number(gap) || 6);
  const colRight = Math.max(pw * 0.06, ...metas.map((box) => box.left + box.width));
  const reserved = Math.min(pw * 0.16, Math.max(colRight, pw * 0.08));
  for (const box of list) {
    if (metas.includes(box)) {
      box.width = Math.min(box.width, reserved);
      continue;
    }
...
/**
 * Post CJK-fit relayout: keep sidebar/body apart, push later boxes down, align author rows.
 * Slight looseness OK; stacked ink not OK. Does not shrink fonts.
 */
...
export function snapMirrorBoxes(boxes = [], pageWidth = 0, pageHeight = 0) {
  const resolved = resolveVerticalMirrorCollisions(boxes, { gap: 2 });
  return resolved.map((box) => ({
    ...
    pageWidth: pageWidth || box.pageWidth,
    pageHeight: pageHeight || box.pageHeight
  }));
}

export function boxesHaveInkOverlap(boxes = [], minArea = 20) {
  const list = (boxes || []).map((box, index) => cloneLayoutBox(box, index));
  const hits = [];
  for (let i = 0; i < list.length; i++) {
    for (let j = i + 1; j < list.length; j++) {
      if (list[i].dir === "vertical" || list[j].dir === "vertical") continue;
      if (list[i].role === "margin" || list[j].role === "margin") continue;
      if (Math.min(boxArea(list[i]), boxArea(list[j])) < 80) continue;
      const left = Math.max(list[i].left, list[j].left);
      const top = Math.max(list[i].top, list[j].top);
      const right = Math.min(list[i].left + list[i].width, list[j].left + list[j].width);
      const bottom = Math.min(list[i].top + list[i].height, list[j].top + list[j].height);
      const area = Math.max(0, right - left) * Math.max(0, bottom - top);
      if (area > minArea) hits.push({ i, j, area, a: list[i], b: list[j] });
    }
  }
  return hits;
}
...
/** Axis-aligned user-space box + writing direction from pdf.js transform. */
export function itemTransformMeta(item = {}) {
  const transform = Array.isArray(item.transform) ? item.transform : [];
  const originX = Number.isFinite(Number(item.x)) ? Number(item.x) : Number(transform[4]) || 0;
  const originY = Number.isFinite(Number(item.y)) ? Number(item.y) : Number(transform[5]) || 0;
  const width = Math.max(0, Number(item.width) || 0);
  const height = Math.max(0, Number(item.height) || 0);
  if (transform.length < 6) {
    return { x: originX, y: originY, width, height, vertical: false, dir: "horizontal", originX, originY };
  }
  const [a, b, c, d] = transform;
  const sx = Math.hypot(a, b);
  const sy = Math.hypot(c, d);
  const vertical = sx > 0 && Math.abs(b) > Math.abs(a) * 1.2;
  if (!vertical || width <= 0) {
    return { x: originX, y: originY, width, height, vertical: false, dir: "horizontal", originX, originY };
  }
  const dirX = a / sx;
  const dirY = b / sx;
  const ascX = sy ? c / sy : 0;
  const ascY = sy ? d / sy : 0;
  const corners = [
    [originX, originY],
    [originX + dirX * width, originY + dirY * width],
    [originX + ascX * height, originY + ascY * height],
    [originX + dirX * width + ascX * height, originY + dirY * width + ascY * height]
  ];
...
export function normalizeMirrorItem(item) {
  ...
  const box = itemTransformMeta(item);
  return {
    str: String(item.str || ""),
    x: box.x,
    y: box.y,
...
export function buildMirrorLayout(input, pageSize = {}) {
  ...
  const rawBoxes = snapMirrorBoxes(
    markAuthorBoxes(linesToMirrorBoxes(lines, pageWidth, pageHeight), pageWidth),
    pageWidth,
    pageHeight
  );
  ...
  const partitioned = partitionFigureTickBoxes(rawBoxes, pageWidth, pageHeight);
  const boxes = partitioned.boxes;
  ...
  const figureVisuals = clipFiguresAroundText(
    coverFigureTickBands(
      findUncoveredRegions(textRects, pageWidth, pageHeight).map((rect) => ({
        kind: "figure",
        rect
      })),
      partitioned.ticks,
...
export function translatableMirrorUnits(layout) {
  return (layout?.translatable || layout?.boxes || [])
    ...
    .map((box) => ({
      ...
      kind: "text",
      dir: box.dir || (box.vertical ? "vertical" : "horizontal"),
      vertical: Boolean(box.vertical || box.dir === "vertical"),
      rect: box.rect,
...
/**
 * Short rotated figure-axis tokens (attention-head ticks), not a page-margin strip.
 * Source AABBs are ~1em wide; CJK forced into those boxes overflows and crushes.
 */
export function looksLikeFigureTickLabel(box, pageWidth = 0, pageHeight = 0) {
  if (!box) return false;
  const vertical = box.vertical === true || box.dir === "vertical";
  if (!vertical) return false;
  const text = String(box.text || box.str || box.translation || box.sourceText || "").trim();
  if (looksLikeArxivMeta(text)) return false;
  const pw = Math.max(1, Number(pageWidth) || 1);
  const ph = Math.max(1, Number(pageHeight) || 1);
  const left = Number(box.rect?.left ?? box.left ?? box.x) || 0;
  const width = Number(box.rect?.width ?? box.width) || 0;
  const height = Number(box.rect?.height ?? box.height) || 0;
  if (left < pw * 0.12 && height > ph * 0.16) return false;
  if (height > ph * 0.22) return false;
  if (width > Math.max(24, pw * 0.055)) return false;
...
/** Drop dense in-figure ticks from the overlay; keep tall left-margin meta. */
export function partitionFigureTickBoxes(boxes = [], pageWidth = 0, pageHeight = 0) {
  const list = Array.isArray(boxes) ? boxes : [];
  const pw = Math.max(1, Number(pageWidth) || 1);
  const ph = Math.max(1, Number(pageHeight) || 1);
  const ticks = list.filter((box) => looksLikeFigureTickLabel(box, pw, ph));
  if (ticks.length < 6) return { boxes: list, ticks: [] };
  return {
    boxes: list.filter((box) => !ticks.includes(box)),
    ticks
  };
}

/** Grow / add figure crops so dropped tick rows stay visible as original ink. */
export function coverFigureTickBands(figures = [], ticks = [], pageWidth = 0, pageHeight = 0) {
  const pw = Math.max(1, Number(pageWidth) || 1);
  const ph = Math.max(1, Number(pageHeight) || 1);
  const out = (figures || []).map((fig) => ({ ...fig, rect: { ...(fig.rect || fig) } }));
  for (const band of clusterFigureTickBands(ticks)) {
    if (band.width < pw * 0.08) continue;
    const hit = out.find((fig) => figureTouchesTickBand(fig.rect, band, pw));
    if (hit) hit.rect = unionRects([hit.rect, band]);
    else if (band.height >= 10) out.push({ kind: "figure", rect: band });
  }
  return out.filter((fig) => {
    const r = fig.rect || {};
    return (r.width || 0) >= pw * 0.08 && (r.height || 0) >= Math.min(16, ph * 0.03);
  });
}

function clusterFigureTickBands(ticks = []) {
  const list = (ticks || []).map((box) => ({ ...mirrorBoxGeom(box), box }));
  if (!list.length) return [];
  list.sort((a, b) => a.top - b.top || a.left - b.left);
  const rows = [];
  for (const tick of list) {
    const row = rows.find((group) => Math.abs(group[0].top - tick.top) <= Math.max(18, group[0].height * 0.85));
    if (row) row.push(tick);
    else rows.push([tick]);
  }
  return rows.map((row) => inflateRect(unionRects(row), 4));
}

function figureTouchesTickBand(rect, band, pageWidth) {
  const g = mirrorBoxGeom(rect);
  const pad = 40;
  return (
    boxesOverlapX({ ...g, left: g.left - 12, width: g.width + 24 }, band, -Math.max(12, pageWidth * 0.04)) &&
    boxesOverlapY({ ...g, top: g.top - pad, height: g.height + pad * 2 }, band)
  );
}

/** Keep figure crops from swallowing titles / captions after tick bands expand. */
export function clipFiguresAroundText(figures = [], boxes = []) {
  const blockers = (boxes || []).filter((box) => box?.rect && box.kind !== "figure");
  return (figures || [])
    .map((fig) => {
      let rect = { ...fig.rect };
      for (const box of blockers) {
        const g = mirrorBoxGeom(box);
        if (!boxesOverlapX(rect, g, 2) || !boxesOverlapY(rect, g, 2)) continue;
        const keepBelow = rect.top + rect.height - (g.top + g.height);
        const keepAbove = g.top - rect.top;
        if (keepBelow >= keepAbove && keepBelow > 12) {
          rect = { ...rect, top: g.top + g.height + 1, height: keepBelow - 1 };
        } else if (keepAbove > 12) {
          rect = { ...rect, height: keepAbove - 1 };
        }
      }
      return { ...fig, rect };
    })
    .filter((fig) => (fig.rect?.width || 0) > 8 && (fig.rect?.height || 0) > 12);
}

export function isMarginMirrorBox(box, pageWidth = 0, pageHeight = 0) {
  if (!box) return false;
  if (box.vertical || box.dir === "vertical" || box.role === "margin") return true;
  const pw = Math.max(1, Number(pageWidth) || 1);
  const ph = Math.max(1, Number(pageHeight) || 1);
  const left = Number(box.rect?.left) || Number(box.left) || 0;
  const width = Number(box.rect?.width) || Number(box.width) || 0;
  const height = Number(box.rect?.height) || Number(box.height) || 0;
  if (looksLikeArxivMeta(box.text || box.translation || box.sourceText)) {
    return left < pw * 0.36 || width < pw * 0.18;
  }
  const leftStrip = left < pw * 0.12 && width < pw * 0.12;
  const tall = height > ph * 0.16 || height > Math.max(24, width * 2.4);
...
export function sortBoxesReadingOrder(boxes, pageWidth = 0) {
  ...
  const ph = Math.max(
    1,
    ...list.map((box) => (Number(box.rect?.top) || 0) + (Number(box.rect?.height) || 0)),
    1
  );
  const margin = [];
  const body = [];
  for (const box of list) {
    if (isMarginMirrorBox(box, pw, ph)) margin.push(box);
    else body.push(box);
  }
  const splitX = detectBoxColumnSplit(body, pw);
  const orderedBody = splitX == null ? sortByTopLeft(body) : sortColumnAware(body, pw, splitX);
  return [...orderedBody, ...sortByTopLeft(margin)];
}

function sortColumnAware(boxes, pageWidth, splitX) {
  const out = [];
  let left = [];
  let right = [];
  const flush = () => {
  ...
  for (const box of sortByTopLeft(boxes)) {
    const width = Number(box.rect?.width) || 0;
    const leftEdge = Number(box.rect?.left) || 0;
    const full = width > pageWidth * 0.58 && leftEdge < splitX && leftEdge + width > splitX;
    if (full) {
      flush();
      out.push(box);
...
export function looksLikeSectionHeading(text) {
  return /^(abstract|introduction|conclusion|references|acknowledgements?|acknowledgment|appendix|related work|background|discussion|limitations|ethics statement|bibliography|contents)\b\.?$/i.test(
    String(text || "").trim()
  );
}
...
export function looksLikeAuthorCell(box, pageWidth) {
  ...
  if (box.vertical || box.dir === "vertical" || box.role === "margin") return false;
  if (box.role === "title" || box.role === "heading" || box.role === "caption") return false;
  const text = String(box.text || "").trim();
  if (!text || looksLikeSectionHeading(text)) return false;
  const pw = Math.max(1, Number(pageWidth) || 1);
  const width = Number(box.rect?.width) || 0;
  if (width > pw * 0.4) return false;
  const cx = (Number(box.rect?.left) || 0) + width / 2;
  const centered = Math.abs(cx - pw / 2) < pw * 0.1;
  if (centered && width < pw * 0.22 && text.split(/\s+/).length <= 3 && !/@/.test(text)) return false;
...
function clusterMirrorLines(items, options = {}) {
  ...
  for (const item of items) {
    if (item.vertical || item.dir === "vertical") verts.push(item);
    else horiz.push(item);
  }
  return [...clusterHorizontalMirrorLines(horiz, options), ...clusterVerticalMirrorStrips(verts)];
}

function clusterHorizontalMirrorLines(items, options = {}) {
...
/** Collinear rotated runs (arXiv strip, figure axis labels) stay one tall box. */
function clusterVerticalMirrorStrips(items) {
  if (!items.length) return [];
  const sorted = [...items].sort((a, b) => a.x - b.x || b.y - a.y);
  const groups = [];
  for (const item of sorted) {
    const group = groups.find((part) => {
      const last = part[part.length - 1];
      const overlap = xOverlapRatio(
        { x: last.x, width: Math.max(last.width, 4) },
        { x: item.x, width: Math.max(item.width, 4) }
      );
      const gap = Math.abs(item.y - (last.y + last.height));
      const em = Math.max(last.width, last.height, item.width, item.height, 8);
      return overlap >= 0.35 && gap <= em * 1.8;
    });
...
function looksLikeGridCellItem(item) {
  const text = String(item?.str || item?.text || "").trim();
  if (!text || text.length > 64) return false;
  if (/@/.test(text)) return true;
  if (/^[∗*†‡§¶0-9]+$/.test(text)) return false;
  if (looksLikeSectionHeading(text)) return false;
  const words = text.split(/\s+/).filter(Boolean);
  if (words.length <= 6 && /^[A-Z\u00c0-\u024f]/.test(text) && !/[.!?]$/.test(text)) return true;
  return words.length <= 4 && text.length <= 32 && !/[.!?]/.test(text);
}
...
function splitLineByGaps(items, options = {}) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    const cellLike = looksLikeGridCellItem(sorted[i - 1]) && looksLikeGridCellItem(sorted[i]);
    const bothMail = /@/.test(sorted[i - 1].str || "") && /@/.test(sorted[i].str || "");
    const cut = bothMail || cellLike
      ? Math.max(8, em * 0.8)
      : options.dense
        ? Math.max(22, em * 2.4)
        : Math.max(14, em * 1.55);
    // Author / table cells leave ~0.8–3em gutters; body stays merged.
    if (gap > cut) {
...
function finalizeMirrorLine(items) {
  const vertical = items.some((item) => item.vertical || item.dir === "vertical");
  const sorted = vertical
    ? [...items].sort((a, b) => b.y + (b.height || 0) - (a.y + (a.height || 0)) || a.x - b.x)
  ...
  for (const item of sorted) {
    if (prev) {
      const gap = vertical
        ? Math.max(0, prev.y - (item.y + (item.height || 0)), item.y - (prev.y + (prev.height || 0)))
        : item.x - (prev.x + prev.width);
      const spaceWidth = Math.max(prev.height || 0, item.height || 0, 8) * (vertical ? 0.05 : 0.22);
      const punct = /^[.,;:!?)]$/.test(item.str);
      if ((vertical ? !punct : gap > spaceWidth) && !/\s$/.test(text) && !/^\s/.test(item.str)) text += " ";
    }
    text += item.str;
    prev = item;
  }
  return {
    ...
    y: vertical ? Math.min(...sorted.map((item) => item.y)) : median(sorted.map((item) => item.y)),
    width: Math.max(...sorted.map((item) => item.x + item.width)) - Math.min(...sorted.map((item) => item.x)),
    height: vertical
      ? Math.max(...sorted.map((item) => item.y + (item.height || 0))) - Math.min(...sorted.map((item) => item.y))
      : Math.max(...sorted.map((item) => item.height || 0)),
...
function linesToMirrorBoxes(lines, pageWidth, pageHeight) {
  ...
  while (remaining.length) {
    ...
    const vertical = group.some((line) => line.vertical || line.dir === "vertical");
    const role = vertical ? "margin" : headingRole(group[0], group[1] || null, stats);
...
function canMergeLines(a, b, stats) {
  if (!a || !b) return false;
  if (a.vertical || b.vertical || a.dir === "vertical" || b.dir === "vertical") return false;
  if (looksLikeSectionHeading(a.text) || looksLikeSectionHeading(b.text)) return false;
  ...
  const leadCap = Math.max(stats.medianLead, stats.medianH) * (narrow ? 1.35 : 1.7);
  if (gap > leadCap) return false;
  ...
  const leftover =
    widthRatio < 0.5 &&
    /^[a-z]/.test(b.text) &&
    Math.min(a.width, b.width) < pageWidth * 0.28 &&
    overlap >= 0.7 &&
    gap <= Math.max(stats.medianLead, stats.medianH) * 1.8;
  if (widthRatio < 0.5 && !leftover) return false;
  return true;
}
...
function mirrorLineStats(lines, pageWidth = 0) {
  ...
  const bodyLeads = leadings.filter((lead) => lead <= medianH * 3.2);
  const use = bodyLeads.length ? bodyLeads : leadings;
  const medianLead = use[Math.floor(use.length / 2)] || medianH * 1.2;
  const maxH = Math.max(0, ...heights);
  return { medianH, medianLead, maxH, pageWidth };
}

function headingRole(line, next, stats) {
  if (line?.vertical || line?.dir === "vertical") return "margin";
...
function isHeadingLine(line, next, stats) {
  ...
  if (line.vertical || line.dir === "vertical") return false;
  if (looksLikeSectionHeading(line.text)) return true;
...
function sortByTopLeft(boxes) {
  return [...(boxes || [])].sort((a, b) => {
    ...
    const band = Math.max(6, Math.min(ah || 24, bh || 24, 28) * 0.7);
...
function detectBoxColumnSplit(boxes, pageWidth) {
  const body = (boxes || []).filter((box) => {
    const width = Number(box.rect?.width) || 0;
    const left = Number(box.rect?.left) || 0;
    if (box.vertical || box.dir === "vertical" || box.role === "margin") return false;
    if (width < pageWidth * 0.12 && left < pageWidth * 0.12) return false;
    return width < pageWidth * 0.55;
  });
