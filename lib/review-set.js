...
export function buildReviewSet(units, options = {}) {
  ...
  const byChosenField = groupBy(chosen, (unit) => unit.field);
  const lists = REVIEW_FIELDS.map((field) => (byChosenField.get(field) || []).slice().sort((a, b) => {
    return Number(a.confidence) - Number(b.confidence) || byStableId(a, b);
  }));
  const ordered = [];
