...
/**
 * A line that is one person (markers allowed) and nothing else.
 * Institution lines and lines that bury a second person return "".
 */
export function authorNameFromLine(line) {
  const raw = String(line || "").replace(/\s+/g, " ").trim();
  if (!raw || /@/.test(raw)) return "";
  const people = personNamesInLine(raw);
  if (people.length !== 1) return "";
  const nameTokens = new Set(people[0].toLowerCase().replace(/\./g, "").split(/\s+/).filter(Boolean));
  for (const token of raw.split(/\s+/)) {
    if (MARKER_ONLY.test(token)) continue;
    const core = cleanToken(token).replace(MARKER_CHARS, "").replace(/\./g, "").toLowerCase();
    if (!core || nameTokens.has(core)) continue;
    return "";
  }
  return raw;
}

/** Person names written into affiliation, other than this author's own name. */
function foreignPeople(author) {
  const own = nameKey(author?.name);
  const seen = new Set();
  const found = [];
  for (const person of personNamesInLine(author?.affiliation)) {
    const key = nameKey(person);
    if (!key || key === own || seen.has(key)) continue;
    const ownParts = own.split(" ").filter(Boolean);
    if (ownParts.length && key.split(" ").every((part) => ownParts.includes(part))) continue;
    seen.add(key);
    found.push(person);
  }
  return found;
...
function nameListsSeveralPeople(value) {
  return personNamesInLine(value).length >= 2;
}
...
const FULL_BYLINE_MIN = 7;
const FULL_BYLINE_MAX = 9;

function soleEmailLine(line) {
  const found = uniqueEmails(line);
  if (found.length !== 1) return "";
  const rest = String(line || "").replace(EMAIL_RE, "").replace(/[\s,;<>()]+/g, "");
  return rest ? "" : String(line || "").trim();
}

function affiliationLine(line) {
  const text = String(line || "").replace(/\s+/g, " ").trim();
  if (!text || soleEmailLine(text) || authorNameFromLine(text)) return "";
  const words = text.split(/\s+/);
  if (words.length > 8) return "";
  if (/[.!?]/.test(text) && words.length > 4) return "";
  return text;
}

/**
 * Deterministic title-block pack from author-region lines.
 * One person per name line. A line that mixes a person into an institution is not a person.
 */
export function harvestTitleStructure(pageText, page) {
  const lines = String(pageText || "").split(/\n/).map((line) => line.trim()).filter(Boolean);
  const abstractAt = lines.findIndex((line) => /^(?:abstract\b|摘要)/i.test(line));
  if (abstractAt < 1) return null;
  const head = lines.slice(0, abstractAt);
  const title = head.find((line) => !soleEmailLine(line) && !authorNameFromLine(line) && !affiliationLine(line) &&
    line.split(/\s+/).length >= 2) || head[0];
  const authors = [];
  let current = null;
  const flush = () => {
    if (!current?.name) return;
    const author = { name: current.name };
    if (current.affiliation) author.affiliation = current.affiliation;
    if (current.email) author.email = current.email;
    authors.push(author);
    current = null;
  };
  for (const line of head) {
    if (line === title) continue;
    const name = authorNameFromLine(line);
    if (name) {
      if (current && nameKey(current.name) === nameKey(name)) continue;
      flush();
      current = { name, affiliation: "", email: "" };
      continue;
    }
    if (!current) continue;
    const email = soleEmailLine(line);
    if (email) {
      current.email = current.email ? `${current.email} ${email}` : email;
      continue;
    }
    const affiliation = affiliationLine(line);
    if (affiliation && !current.affiliation) current.affiliation = affiliation;
  }
  flush();
  const body = [];
  for (const line of lines.slice(abstractAt + 1)) {
    if (/^(?:\d+\s+)?(?:introduction|related work|background|references)\b/i.test(line)) break;
    body.push(line);
    if (body.join(" ").length > 1800) break;
  }
  return validatePdfStructure({
    version: PDF_STRUCTURE_VERSION,
    ...(Number(page) > 0 ? { page: Number(page) } : {}),
    title,
    authors,
    abstract: { heading: lines[abstractAt].replace(/\s+/g, " ").trim(), body: body.join(" ") }
  });
}

function harvestedFullByline(pageText, page) {
  const harvested = harvestTitleStructure(pageText, page);
  if (!harvested?.ok) return null;
  const count = harvested.structure.authors.length;
  if (count < FULL_BYLINE_MIN || count > FULL_BYLINE_MAX) return null;
  if (structureAuthorsLookTruncated(harvested.structure.authors, pageText)) return null;
  return harvested;
}

/** Byline cells may become the author grid only when the pack itself is clean and complete. */
export function authorBylineGridOk(cells, pageText) {
  const list = Array.isArray(cells) ? cells.filter((cell) => cell && typeof cell === "object") : [];
  if (!list.length || list.some((cell) => !String(cell.name || "").trim())) return false;
  const prepared = list.map((cell) => ({
    name: String(cell.name || ""),
    affiliation: String(cell.affiliation || ""),
    email: String(cell.email || ""),
    markers: ""
  }));
  if (dirtyAuthorIndexes(prepared).size) return false;
  if (prepared.some((author) => foreignPeople(author).length >= 2)) return false;
  if (pageText && structureAuthorsLookTruncated(prepared, pageText)) return false;
  return true;
}

/**
 * One STRUCTURE call, then a single completeness retry when the pack is dirty
 * or clean but the title-page text still shows at least two omitted people.
 * A second dirty or truncated pack is discarded. If the author-region lines still
 * yield a clean full byline (7–9 people), that pack is used instead of the failure.
 * Attention surnames are not hardcoded.
 */
export async function resolveTitleStructure(pageText, page, request) {
  ...
  const truncated = first.ok && structureAuthorsLookTruncated(first.structure.authors, pageText);
  const dirty = !first.ok && first.reason === "authors-dirty";
  if (first.ok && !truncated) return first;
  if (!truncated && !dirty) return harvestedFullByline(pageText, page) || first;
  const covered = first.ok ? first.structure.authors : [];
  const missing = missingTitleAuthors(covered, pageText);
  const second = await ask(structureCompletenessPrompt(pageText, page, missing));
  if (second.ok && !structureAuthorsLookTruncated(second.structure.authors, pageText)) return second;
  const harvested = harvestedFullByline(pageText, page);
  if (harvested) return harvested;
  if (!second.ok) return fail(second.reason === "authors-dirty" ? "authors-dirty" : "authors-truncated");
  return fail("authors-truncated");
}
...
export function validatePdfStructure(input) {
  ...
  const absorbed = prepared.some((author) => foreignPeople(author).length >= 2);
  if (dirty.size >= 2 || absorbed) return fail("authors-dirty");
  const authors = [];
...
function dirtyAuthorIndexes(prepared) {
  ...
  prepared.forEach((author, index) => {
    ...
    if (nameListsSeveralPeople(author.name) || foreignPeople(author).length) dirty.add(index);
    const ownMail = new Set(uniqueEmails(author.email));
    if (uniqueEmails(`${author.name}\n${author.affiliation}`).some((mail) => !ownMail.has(mail))) dirty.add(index);
  });
  prepared.forEach((author, index) => {
...
/** Translatable strings in refill order. Author name, affiliation, email, and markers stay on the source pack. */
...
/** Refill string fields. Author name, affiliation, email, and markers stay on the source pack. */
