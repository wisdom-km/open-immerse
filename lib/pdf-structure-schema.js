...
/** Translatable strings in refill order. Person names, email, and markers stay on the source pack. */
...
function writeSlot(structure, path, value) {
  ...
  if (path[0] === "title") structure.title = text;
    ...
    if (!author || path[2] === "name" || path[2] === "email" || path[2] === "markers") return;
    if (path[2] === "affiliation") author[path[2]] = text;
...
/** Refill string fields. Shape, roles, person names, emails, and markers stay put. */
