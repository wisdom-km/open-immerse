...
| 行内公式 | **基线嵌进句中**；简单关系（`N=6`、`h=8`、`P_drop=0.1`、`ε_ls=0.1`）写 Unicode，不发 ⟦fN⟧、不裁图；下标 / 根号 / ∑ / 矩阵 / PE 仍 pageRaster |
...
| 宽度 | 页宽占比不要直接写成正文列的 `%`。正文列已经扣过左右 `0.085`，宽度用 `pageFraction / (1 − 2×0.085)`，纸比左页窄时再乘左页宽 / 纸宽，最后夹在列宽内。宽应明显大于合前约 48% 列宽。列宽仍优先。量测墨迹硬门 ≥ **1.0×** `bodyFs`（专有 display 偏好 ≥ **1.2×**）。#63 的 ≥ **1.6×** 仍是修宽后的强期望（盒 `2em` × 约 0.8 share；自然到约 3× 且 ≤ **5×** 仍通过）。短裁图若实测 share 让墨迹低于 1.2×，只把这一张的 floor 抬到 `1.2 / inkShare`，不先把图拉满整列，也不动 #62 pad |
...
| 高度 | **墨迹**（不是裁图盒）≥ **1.0 ×** 右栏中文 `bodyFs`，目标 **1.05～1.15 ×**。#62 行内白边（4–5px）在盒内，把盒钉在 `1.22em` 时墨迹只有约 0.6–0.9×。盒高 = `targetInk / inkShare`（名义墨迹 16px、行内 pad 4px → share 16/24 → 盒 **1.65em**，约 1.55–1.8em）。旧 `max-height: 1.45em` 会挡住这扇门，已撤 |
...
.oi-pdf-inline-math img {
  display: block;
  height: var(--oi-pdf-inline-crop-em, 1.65em); /* 墨迹目标 ÷ inkShare，不是把 1.22 改成另一个常数 */
  width: auto;
...
| 行内公式 | 量测墨迹 **1.05～1.15×** 正文（硬门 ≥ **1.0×**）；盒按 inkShare 反推，名义 **1.65em**；仍嵌在句中 |
