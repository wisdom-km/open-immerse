...
| 宽度 | 页宽占比不要直接写成正文列的 `%`。正文列已经扣过左右 `0.085`，宽度用 `pageFraction / (1 − 2×0.085)`，纸比左页窄时再乘左页宽 / 纸宽，最后夹在列宽内。宽应明显大于合前约 48% 列宽。墨迹高 / 正文硬门 ≥ **1.6×**（可读带 **1.6～2.0×**；修宽后自然到约 3× 且 ≤ **5×** 仍通过，不把 2.5～3.5× 当失败）。仍低于 1.6× 才用 `min-height: 2em`，不先把图拉满整列 |
