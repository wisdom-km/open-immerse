...
停住以后不再外扩。邻行字盒即使和下标差一丝重叠，也不把外扩推进那一行的中心。不要用固定的「整行加 0.01 页高」去盖住上一行正文或 Fig.2。子公式彼此也是障碍，两行显示公式不能互相吃进上下标。第 7 页 Adam 句里的 β、ε 留在同一段；学习率公式是后面单独的居中块，不把 `p07-lr-formula.png` 那种整行白块嵌进句子。
...
- 独占公式：`margin: 10px 0 14px`，水平居中。宽度是页宽占比除掉正文列左右边（`pageFraction / 0.83`），纸比左页窄时再向左页对齐，夹在列宽内；仍矮则 `min-height: 2.25em`。无描边、无阴影。白边目标约 6 CSS px 以内。
